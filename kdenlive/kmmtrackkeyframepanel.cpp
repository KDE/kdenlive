...
KMMTrackKeyFramePanel::KMMTrackKeyFramePanel(KTimeLine *timeline,
						...
						QWidget *parent,
						const char *name) :
					KMMTrackPanel(timeline, doc, new KTrackPlacer(doc, timeline, docTrack), type, parent,name)
{
	m_type = type;
	m_trackIsCollapsed = isCollapsed;
	uint widgetHeight;
	if (type == EFFECTKEYFRAMETRACK) widgetHeight = 50;
...
void KMMTrackKeyFramePanel::resizeTrack()
{
	//clearViewDecorators();
	uint widgetHeight;
	if (m_type == EFFECTKEYFRAMETRACK) widgetHeight = 50;
	else widgetHeight = 30;
