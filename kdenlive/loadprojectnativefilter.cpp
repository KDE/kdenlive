...
bool LoadProjectNativeFilter::load(QFile &file, KdenliveDoc *document)
{
	...
	while(!n.isNull()) {
		...
		if(!e.isNull()) {
			if( (e.tagName() == "AVFileList") || (e.tagName() == "avfilelist")) {
				if(!avListLoaded) {
					avListLoaded = true;
...
void LoadProjectNativeFilter::loadTrackList(QDomElement &element, KdenliveDoc *document)
{
	...
	if(clip) {
		if(clip->isProjectClip()) {
			document->setProjectClip(dynamic_cast<DocClipProject *>(clip));
		} else {
			delete clip;
			kdError() << "Base clip detected, not a project clip. Ignoring..." << endl;
		}
	} else {
		kdError() << "Could not create clip for track list, ignoring..." << endl;
	}
}

void LoadProjectNativeFilter::addToDocument(const QString &parent, QDomElement &clip, KdenliveDoc *document)
{
