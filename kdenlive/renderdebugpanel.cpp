...
RenderDebugPanel::RenderDebugPanel(QWidget * parent,
    const char *name):QVBox(parent, name), m_mainLayout(this,
    "mainLayout"), m_rendererList(&m_mainLayout, "list"),
m_textLayout(QSplitter::Vertical, &m_mainLayout, "textLayout"),
m_widgetStack(&m_textLayout, "stack"), m_vemlEdit(&m_textLayout,
    "vemlEdit"), m_buttonLayout(this, "button layout"),
m_spacer(&m_buttonLayout, "spacer"),
m_ignoreMessages(i18n("Ignore Incoming Messages "), &m_buttonLayout,
    "ignore messages"), m_sendVemlButton(i18n("Send Veml Command"),
    &m_buttonLayout, "ignore messages"),
m_saveMessages(i18n("Save Messages"), &m_buttonLayout, "save button"),
m_nextId(0)
{
    m_spacer.setSizePolicy(QSizePolicy(QSizePolicy::Expanding,
	    QSizePolicy::Maximum));
    m_rendererList.setSizePolicy(QSizePolicy(QSizePolicy::Maximum,
	    QSizePolicy::Expanding));
    m_widgetStack.setSizePolicy(QSizePolicy(QSizePolicy::Expanding,
	    QSizePolicy::Expanding));
    m_vemlEdit.setSizePolicy(QSizePolicy(QSizePolicy::Expanding,
	    QSizePolicy::Expanding));
    connect(&m_rendererList, SIGNAL(highlighted(int)), &m_widgetStack,
	SLOT(raiseWidget(int)));
    connect(&m_saveMessages, SIGNAL(clicked()), this,
	SLOT(saveMessages()));
    connect(&m_sendVemlButton, SIGNAL(clicked()), this,
	SLOT(sendDebugVeml()));
    m_ignoreMessages.setChecked(true);

    //Debug Panel Tooltips
    QToolTip::add(&m_saveMessages,
	i18n("Save debug messages from renderer and Kdenlive"));
    QToolTip::add(&m_sendVemlButton,
	i18n("Send VEML messages to the renderer"));
    if (m_ignoreMessages.isChecked()) {
	QToolTip::add(&m_ignoreMessages,
	    i18n("Unselect to ignore debug messages"));
    } else {
	QToolTip::add(&m_ignoreMessages,
	    i18n("Select to ignore debug messages"));
    }
}
...
void RenderDebugPanel::slotPrintRenderDebug(const char *&name,
    const QString & message)
{
    slotPrintDebug(name, message);
}

void RenderDebugPanel::slotPrintRenderWarning(const char *&name,
    const QString & message)
{
    slotPrintWarning(name, message);
}

void RenderDebugPanel::slotPrintRenderError(const char *&name,
    const QString & message)
{
...
void RenderDebugPanel::slotPrintWarning(const char *&name,
    const QString & message)
{
    if (m_ignoreMessages.isChecked())
	return;
...
void RenderDebugPanel::slotPrintError(const char *&name,
    const QString & message)
{
    if (m_ignoreMessages.isChecked())
	return;
...
void RenderDebugPanel::slotPrintDebug(const char *&name,
    const QString & message)
{
    if (m_ignoreMessages.isChecked())
	return;
...
void RenderDebugPanel::saveMessages()
{
    QString save;
    KURL url =
	KFileDialog::getSaveURL(0, 0, this,
	i18n("Save Debug Messages..."));
	...
	if (file.open(IO_WriteOnly)) {
		...
		QTextEdit *edit =
		    (QTextEdit *) m_widgetStack.widget(count);
		save = "*******************************************\n";
		save = save + "* " + edit->name() + "\n" +
		    "*******************************************\n\n" +
		    edit->text() +
...
{
    emit debugVemlSendRequest(m_rendererList.currentText(),
	m_vemlEdit.text());
}
