/***************************************************************************
                       TrackPanelClipSlipFunction.cpp  -  description
                          -------------------
 ...
 *                                                                         *
 ***************************************************************************/
#include "trackpanelclipslipfunction.h"
...
TrackPanelClipSlipFunction::TrackPanelClipSlipFunction( Gui::KdenliveApp *app, Gui::KTimeLine *timeline, KdenliveDoc *document) :
		m_app( app ),
		m_timeline( timeline ),
		m_document( document ),
...
TrackPanelClipSlipFunction::~TrackPanelClipSlipFunction()
{}

bool TrackPanelClipSlipFunction::mouseApplies( Gui::KTrackPanel *panel, QMouseEvent *event ) const
{
	return mouseApplies(event->pos());
}

bool TrackPanelClipSlipFunction::mouseApplies( const QPoint &pos) const
{
	...
	if(panel) {
		if(panel->hasDocumentTrackIndex()) {
			...
			if(track) {
				GenTime mouseTime( m_timeline->mapLocalToValue( pos.x() ), m_document->framesPerSecond() );
...
QCursor TrackPanelClipSlipFunction::getMouseCursor( Gui::KTrackPanel *panel, QMouseEvent *event )
{
	return QCursor( Qt::ArrowCursor );
}

bool TrackPanelClipSlipFunction::mousePressed( Gui::KTrackPanel *panel, QMouseEvent *event )
{
	bool result = false;
...
bool TrackPanelClipSlipFunction::mouseReleased( Gui::KTrackPanel *panel, QMouseEvent *event )
{
	bool result = false;
	...
	if(panel->hasDocumentTrackIndex()) {
		...
		if(track) {
			if ( m_clipUnderMouse ) {
				if ( event->state() & Qt::ControlButton ) {} else if ( event->state() & Qt::ShiftButton ) {} else {
					GenTime mouseTime( m_timeline->mapLocalToValue( event->x() ), m_document->framesPerSecond() );
...
bool TrackPanelClipSlipFunction::mouseMoved( Gui::KTrackPanel *panel, QMouseEvent *event )
{
	bool result = false;
...
// virtual
bool TrackPanelClipSlipFunction::dragEntered ( Gui::KTrackPanel *panel, QDragEnterEvent *event )
{
...
// virtual
bool TrackPanelClipSlipFunction::dragMoved ( Gui::KTrackPanel *panel, QDragMoveEvent *event )
{
...
int TrackPanelClipSlipFunction::trackUnderPoint( const QPoint &pos )
{
	uint y = pos.y();
...
// virtual
bool TrackPanelClipSlipFunction::dragLeft( Gui::KTrackPanel *panel, QDragLeaveEvent *event )
{
	if ( !m_selection.isEmpty() ) {
...
// virtual
bool TrackPanelClipSlipFunction::dragDropped ( Gui::KTrackPanel *panel, QDropEvent *event )
{
	if(ClipDrag::canDecode(event)) {
		if ( !m_selection.isEmpty() ) {
			m_selection.setAutoDelete( true );
		...
		if(panel) {
			DocTrackBase *track = m_document->track(panel->documentTrackIndex());
			if(track) {
				GenTime mouseTime( m_timeline->mapLocalToValue( event->pos().x() ), m_document->framesPerSecond() );
				clipUnderMouse =   track->getClipAt( mouseTime );
			}
		}
		...
		if(clipUnderMouse) {
			...
			if(effect) {
				m_app->addCommand( Command::KAddEffectCommand::appendEffect( m_document, clipUnderMouse, effect ), true );
			} else {
				kdWarning() << "EffectDrag::decode did not return an effect, ignoring drag drop..." << endl;
			}
			delete effect;
		}
	}
...
bool TrackPanelClipSlipFunction::moveSelectedClips( int newTrack, GenTime start )
{
...
void TrackPanelClipSlipFunction::addClipsToTracks( DocClipRefList &clips, int track, GenTime value, bool selected )
{
...
void TrackPanelClipSlipFunction::setupSnapToGrid()
{
	...
	QValueVector<GenTime> cursor = m_document->getSnapTimes( m_timeline->snapToBorders(),
	                            								m_timeline->snapToMarkers(),
				    								false,
				    								true);
...
void TrackPanelClipSlipFunction::initiateDrag( DocClipRef *clipUnderMouse, GenTime mouseTime )
{
	m_masterClip = clipUnderMouse;
	m_clipOffset = mouseTime - clipUnderMouse->trackStart();

	m_moveClipsCommand = new Command::KMoveClipsCommand( m_document, m_masterClip );
	m_deleteClipsCommand = Command::KAddRefClipCommand::deleteSelectedClips( m_document );
	setupSnapToGrid();
...
KMacroCommand * TrackPanelClipSlipFunction::createAddClipsCommand()
{
	...
	for ( int count = 0; count < m_document->numTracks(); ++count ) {
		...
		while ( itt.current() ) {
			Command::KAddRefClipCommand * command = new Command::KAddRefClipCommand( m_document->effectDescriptions(), m_document->clipManager(), &m_document->projectClip(), itt.current(), true );
