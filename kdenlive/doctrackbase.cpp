...
QPtrListIterator < DocClipRef > DocTrackBase::endClip(GenTime startValue,
    GenTime endValue, bool selected)
{
...
void DocTrackBase::resizeClipTrackStart(DocClipRef * clip,
    GenTime newStart)
{
    if (!clipExists(clip)) {
	kdError() <<
...
void DocTrackBase::deleteEffectFromClip(const GenTime & position,
    int effectIndex)
{
    DocClipRef *clip = getClipAt(position);
    if (clip) {
