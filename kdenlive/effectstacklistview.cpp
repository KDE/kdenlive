...
void EffectStackListView::dragDropped(QDropEvent* e, QListViewItem* parent, QListViewItem* after)
{
		...
		if(effect) {
			...
			while(itemItt) {
				if ( after == itemItt ) {
					...
					m_app->addCommand(Command::KAddEffectCommand::insertEffect(m_document, m_clip, itemIx+1, effect));
					break;
				}
				++itemIx;;
				itemItt = itemItt->nextSibling();
			...
			kdWarning() << "EffectStackListView::dragDropped() could not decode QDropEvent" << endl;
		}
	} else {
		kdWarning() << "EffectStackListView::dragDropped() a non-EffectDrag compatable QDropEvent" << endl;
	}
}
...
void EffectStackListView::slotDeleteEffect()
{
	int selectedIx = selectedEffectIndex();
	if(selectedIx != -1) {
		m_app->addCommand(Command::KAddEffectCommand::removeEffect(m_document, m_clip, selectedIx));
	}
}
