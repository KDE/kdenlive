...
{
	//video info layout
	QHGroupBox *m_videoGBox = new QHGroupBox( i18n( "Video Stream" ), this, "videoStream" );
...
void ClipPropertiesDialog::setClip(DocClipRef *clip)
{
	...
	if( m_clip )
	{
		...
		if( m_clip->durationKnown() )
		{
			...
			int minutes = (int)floor(m_clip->duration().seconds()/60);
			...
			length.append(QString::number(hours).rightJustify(1, '0', FALSE));
			length.append(":");
