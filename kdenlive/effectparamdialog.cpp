...
namespace Gui {
    ...
    EffectParamDialog::EffectParamDialog(KdenliveApp * app,
	KdenliveDoc * document, QWidget * parent,
	const char *name):QVBox(parent, name), m_desc(0), m_app(app),
	m_document(document) {
	KIconLoader loader;
	 ...
	 m_presetAdd =
	    new KPushButton(QString::null, m_presetLayout, "preset_add");
	 m_presetDelete =
	    new KPushButton(QString::null, m_presetLayout,
	    "preset_delete");

	 m_presetAdd->setIconSet(QIconSet(loader.loadIcon("filenew",
		    KIcon::Toolbar)));
	 m_presetDelete->setIconSet(QIconSet(loader.loadIcon("editdelete",
		    KIcon::Toolbar)));
	...
	slotSetEffectDescription(const EffectDesc & desc) {
	clearEffect();
	m_desc = &desc;
	m_clip = NULL;
	m_effect = NULL;
	...
	for (uint count = 0; count < m_desc->numParameters(); ++count) {
	    ...
	    assert(paramDesc);
	    KTrackPanel *panel =
		paramDesc->createClipPanel(m_app, m_timeline, m_document,
		m_clip);
	    assert(panel);
    ...
    void EffectParamDialog::slotSetEffect(DocClipRef * clip,
	Effect * effect) {
	kdWarning() << "++++++++++ EffectParamDialog::slotSetEffect" <<
	    endl;
	assert(effect);
	clearEffect();
