...
void AudioLevelLayoutState::calculateLayout()
{
    ...
    if (m_config.orientation == Qt::Horizontal) {
        m_offsetWithLabels = m_config.fontMetrics.height() + MARGIN_BETWEEN_LABEL_AND_LEVELS;
    } else {
        m_offsetWithLabels = m_config.fontMetrics.boundingRect(QStringLiteral("-45")).width() + MARGIN_BETWEEN_LABEL_AND_LEVELS;
    }
    ...
    if (m_config.tickLabelsMode == AudioLevel::TickLabelsMode::HideIfSpaceIsLimited) {
        ...
        int drawableSize = fullSize - m_offsetWithLabels;
        ...
        if (!m_shouldDrawLabels) {
            m_offset = TICK_MARK_LENGTH; // Reserve space just for tick marks
        } else {
            m_offset = m_offsetWithLabels;
        }
    } else {
        m_offset = m_offsetWithLabels;
    }
...
int AudioLevelLayoutState::getEffectiveOffset() const
{
    if (isInHoverLabelMode()) {
        return m_offsetWithLabels;
    }
    return m_offset;
}
...
{
    int fullSize = m_config.orientation == Qt::Horizontal ? m_config.widgetSize.height() : m_config.widgetSize.width();
    int drawableSize = fullSize - m_offset;
    int channelSize = (drawableSize - (m_config.audioChannels + 1) * CHANNEL_BORDER_WIDTH) / m_config.audioChannels;
...
int AudioLevelRenderer::channelToSecondaryOffset(int channelIndex, int secondaryAxisLength, int offset, Qt::Orientation orientation)
{
    if (orientation == Qt::Horizontal) {
        // Start at top, do not add offset
        return CHANNEL_BORDER_WIDTH + channelIndex * (secondaryAxisLength + CHANNEL_BORDER_WIDTH);
    } else {
        return offset + CHANNEL_BORDER_WIDTH + channelIndex * (secondaryAxisLength + CHANNEL_BORDER_WIDTH);
    }
}
...
{
    painter.save();
    int effectiveOff = data.layoutState.getEffectiveOffset();
    ...
    if (data.orientation == Qt::Horizontal && data.layoutState.isInHoverLabelMode()) {
        ...
        QRect clipRect(0, 0, data.layoutState.getWidgetSize().width(), data.layoutState.getWidgetSize().height() - effectiveOff);
    ...
    if (data.orientation == Qt::Horizontal) {
        ...
        QRectF drawingRect = PainterUtils::adjustedForPen(QRectF(effectiveOff, 0, totalWidth, totalHeight), pen.widthF());
        if (fillBackground && channelBackgroundColor.isValid()) {
            painter.fillRect(drawingRect, channelBackgroundColor);
        }
        painter.drawRect(drawingRect);
        ...
        for (int i = 1; i < data.audioChannels; ++i) {
            qreal x = effectiveOff + i * (channelWidth + CHANNEL_BORDER_WIDTH);
...
void AudioLevelRenderer::drawDbScale(QPainter &painter, const RenderData &data) const
{
    ...
    if (data.layoutState.isInHoverLabelMode()) {
        ...
        if (data.orientation == Qt::Horizontal) {
            ...
            painter.fillRect(QRectF(0, 0, effectiveOff, data.layoutState.getWidgetSize().height()), bgColor);
        }
    }
    ...
    if (data.orientation == Qt::Horizontal) {
        ...
        qreal y = qMin(data.audioChannels * data.secondaryAxisLength + (data.audioChannels + 1) * CHANNEL_BORDER_WIDTH,
                       data.layoutState.getWidgetSize().height() - effectiveOff);
        ...
        int prevY = -1;
        int y = 0;
        qreal x = effectiveOff - TICK_MARK_LENGTH;
        ...
        for (int i = 0; i < dbLabelCount; i++) {
            ...
            if (drawLabels) {
                ...
                if (prevY < 0 || y - (prevY + labelHeight) > labelMargin) {
                    const QString label = QString::asprintf("%d", value);
                    painter.drawText(QRectF(0, y, effectiveOff - MARGIN_BETWEEN_LABEL_AND_LEVELS, labelHeight), label, QTextOption(Qt::AlignRight));
                    prevY = y;
                }
            }
        }
    ...
    if (data.orientation == Qt::Horizontal) {
        ...
        for (int x = BLOCK_LINE_SPACING; x < data.primaryAxisLength; x = x + BLOCK_LINE_SPACING) {
            for (int i = 0; i < data.audioChannels; i++) {
                ...
                qreal offset = CHANNEL_BORDER_WIDTH + i * (secondaryLen + CHANNEL_BORDER_WIDTH);
                qreal lineStart = offset + blockLineGap - HIDPI_OFFSET_ADJUSTMENT;
        ...
        for (int y = CHANNEL_BORDER_WIDTH + data.primaryAxisLength - BLOCK_LINE_SPACING; y > CHANNEL_BORDER_WIDTH; y -= BLOCK_LINE_SPACING) {
            for (int i = 0; i < data.audioChannels; ++i) {
                ...
                qreal offset = effectiveOff + CHANNEL_BORDER_WIDTH + i * (secondaryLen + CHANNEL_BORDER_WIDTH);
                qreal lineStart = offset + blockLineGap - HIDPI_OFFSET_ADJUSTMENT;
                qreal lineEnd = lineStart + blockLineLength + HIDPI_LENGTH_ADJUSTMENT;
                if (lineStart < lineEnd) {
        ...
        for (int i = 0; i < data.audioChannels; i++) {
            ...
            qreal offset = data.layoutState.getOffset() + CHANNEL_BORDER_WIDTH + i * (secondaryLen + CHANNEL_BORDER_WIDTH);
            qreal lineStart = offset + blockLineGap - HIDPI_OFFSET_ADJUSTMENT;
            qreal lineEnd = lineStart + blockLineLength + HIDPI_LENGTH_ADJUSTMENT;
            if (lineStart < lineEnd) {
                painter.drawLine(PainterUtils::adjustedHorizontalLine(y, lineStart, lineEnd, pen.widthF()));
...
void AudioLevelRenderer::drawChannelPeakIndicator(QPainter &painter, const RenderData &data, int channelIndex) const
{
    ...
    qreal secondaryOffset =
        channelToSecondaryOffset(channelIndex, data.secondaryAxisLength, data.layoutState.getOffset(), data.orientation) - HIDPI_OFFSET_ADJUSTMENT;
...
void AudioLevelRenderer::drawChannelLevelsSolid(QPainter &painter, const RenderData &data) const
{
    ...
    for (int i = 0; i < data.audioChannels; i++) {
        ...
        qreal secondaryOffset = channelToSecondaryOffset(i, data.secondaryAxisLength, data.layoutState.getOffset(), data.orientation) - HIDPI_OFFSET_ADJUSTMENT;
        qreal secondaryLength = data.secondaryAxisLength + HIDPI_LENGTH_ADJUSTMENT;
