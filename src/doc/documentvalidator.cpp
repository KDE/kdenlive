                ...
                for (int j = 0; j < producers.count(); j++) {
                    if (producers.at(j).nodeName() == QLatin1String("blank")) {
                        ...
                        if (!hasVideo) {
                            ...
                            itemb.insert(QLatin1String("data"),
                                         QJsonValue(QStringLiteral("%1:%2").arg(duplicate_playlist.attribute(QStringLiteral("id"))).arg(pos)));
        ...
        if (json.isArray()) {
            ...
            for (int i = 0; i < subtitlesList.size(); i++) {
                if (timelineUuid == documentUuid) {
                    ...
                    path = (i > 0)
                               ? (QFileInfo(m_url.toLocalFile()).dir().absoluteFilePath(QStringLiteral("%1%2-%3.srt").arg(fileName).arg(timelineUuid).arg(i)))
