...
bool DocumentValidator::validate(const double currentVersion)
{
...
#else
        initEffects::parseEffectFiles(pCore->binController()->mltRepository(), setlocale(LC_NUMERIC_MASK, Q_NULLPTR));
#endif
    }
    double version = -1;
    if (kdenliveDoc.isNull() || !kdenliveDoc.hasAttribute(QStringLiteral("version"))) {
        ...
        if (!ok) {
            ...
            if (!ok) version = kdenliveDoc.attribute(QStringLiteral("version")).toDouble(&ok);
            if (!ok) {
...
#define KEEP_TRACK_ORDER 1
        ...
        for (int i = 0; i < max; ++i) {
            ...
            if (duration > 0) prod.setAttribute(QStringLiteral("out"), QString::number(duration));
...
#define LOOKUP_FOLDER 1
    ...
    if (version <= 0.88) {
        ...
        for (int i = 0; i < markers.count(); ++i) {
            ...
            QDomElement prop = m_doc.createElement(QStringLiteral("property"));
            prop.setAttribute(QStringLiteral("name"), "kdenlive:marker." + marker.attribute(QStringLiteral("id")) + ":" + marker.attribute(QStringLiteral("time")));
            QDomText val = m_doc.createTextNode(marker.attribute(QStringLiteral("type")) + ":" + marker.attribute(QStringLiteral("comment")));
            prop.appendChild(val);
            main_playlist.appendChild(prop);
        }
        ...
        for (int i = 0; i < guides.count(); ++i) {
            ...
            QDomElement prop = m_doc.createElement(QStringLiteral("property"));
            prop.setAttribute(QStringLiteral("name"), "kdenlive:guide." + guide.attribute(QStringLiteral("time")));
        ...
        for (int i = 0; i < folders.count(); ++i) {
            QDomElement folder = folders.at(i).toElement();
            QDomElement prop = m_doc.createElement(QStringLiteral("property"));
            prop.setAttribute(QStringLiteral("name"), "kdenlive:folder.-1." + folder.attribute(QStringLiteral("id")));
        ...
        for (int i = 0; i < entries.count(); i++) {
            ...
            if (entryId == QLatin1String("black")) continue;
        ...
        for (int i = 0; i < producers.count(); ++i) {
            QDomElement prod = producers.at(i).toElement();
            QString id = prod.attribute(QStringLiteral("id"));
            if (id == QLatin1String("black")) continue;
        ...
        for (int j = 0; j < kdenlive_producers.count(); j++) {
            ...
            for (int k = 0; k < mlt_producers.count(); k++) {
                ...
                if (mltprod.tagName() != QLatin1String("producer")) continue;
...
void DocumentValidator::checkOrphanedProducers()
{
    ...
    for (int k = 0; k < bin_producers.count(); k++) {
        ...
        if (mltprod.tagName() != QLatin1String("entry")) continue;
