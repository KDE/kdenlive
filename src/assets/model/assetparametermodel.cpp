...
void AssetParameterModel::internalSetParameter(const QString name, const QString paramValue, const QModelIndex &paramIndex)
{
    ...
    if (conversionSuccess) {
        ...
        std::shared_ptr<KeyframeModel> km = nullptr;
        if (m_keyframes) {
...
QJsonDocument AssetParameterModel::toJson(QMap<int, QList<int>> selection, bool includeFixed, bool percentageExport) const
{
    QJsonArray list;
    if (includeFixed && selection.isEmpty()) {
        for (const auto &fixed : m_fixedParams) {
