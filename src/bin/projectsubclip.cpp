    ...
    , m_masterClip(parent)
{
    ...
    ClipLoadTask::start(ObjectId(ObjectType::BinClip, m_parentClipId.toInt(), QUuid()), QDomElement(), true, in, out, this);
}
...
void ProjectSubClip::getThumbFromPercent(int percent)
{
    ...
    if (ThumbnailCache::get()->hasThumbnail(m_parentClipId, m_inPoint + framePos)) {
        ...
        CacheTask::start(ObjectId(ObjectType::BinClip, m_parentClipId.toInt(), QUuid()), 30, m_inPoint, m_outPoint, this);
    }
}
