...
public:
    ...
    MaskTask(const ObjectId &owner, QMap<int, QString> maskProperties, int in, int out, QObject *object);
    ~MaskTask() override;
    static void start(const ObjectId &owner, QMap<int, QString> maskProperties, int in = 0, int out = 0, QObject *object = nullptr);
