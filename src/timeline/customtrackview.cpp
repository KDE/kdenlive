...
void CustomTrackView::updateEffect(int track, GenTime pos, QDomElement insertedEffect, bool updateEffectStack, bool replaceEffect)
{
    ...
    if (clip) {
        ...
        if (reloadFades) clip->setSelectedEffect(clip->selectedEffectIndex());
...
void CustomTrackView::updateTransition(int track, const GenTime &pos, const QDomElement &oldTransition, const QDomElement &transition, bool updateTransitionWidget)
{
    ...
    if (oldTransition.attribute("transition_atrack") != transition.attribute("transition_atrack") || oldTransition.attribute("transition_btrack") != transition.attribute("transition_btrack"))
        force = true;
    m_timeline->transitionHandler->updateTransition(oldTransition.attribute("tag"), transition.attribute("tag"), transition.attribute("transition_btrack").toInt(), transition.attribute("transition_atrack").toInt(), item->startPos(), item->endPos(), transition, force);
...
void CustomTrackView::mouseReleaseEvent(QMouseEvent * event)
{
    ...
    if (m_moveOpMode == MoveOperation) {
        ...
        int fadeIndex2 = item->hasEffect("volume", "fadein");
        ...
        int fadeIndex2 = item->hasEffect("volume", "fadeout");
        if (fadeIndex >= 0 && fadeIndex2 >= 0) {
            // We have 2 fadin effects, use currently selected or first one
            int current = item->selectedEffectIndex();
            if (fadeIndex != current) {
...
void CustomTrackView::updatePositionEffects(ClipItem* item, const ItemInfo &info, bool standalone)
{
    ...
    if (effectPos != -1) {
        ...
        int start = item->cropStart().frames(m_document->fps());
        int duration = EffectsList::parameter(effect, "out").toInt() - EffectsList::parameter(effect, "in").toInt();
        ...
        duration += start;
        EffectsList::setParameter(effect, "in", QString::number(start));
        EffectsList::setParameter(effect, "out", QString::number(duration));
    ...
    effectPos = item->hasEffect("volume", "fadeout");
    if (effectPos != -1) {
    ...
    effectPos = item->hasEffect("freeze", "freeze");
    if (effectPos != -1) {
...
void CustomTrackView::autoTransition()
{
    ...
    m_timeline->transitionHandler->updateTransition(transition.attribute("tag"), transition.attribute("tag"), transition.attribute("transition_btrack").toInt(), transition.attribute("transition_atrack").toInt(), tr->startPos(), tr->endPos(), transition);
    setDocumentModified();
}
...
void CustomTrackView::slotAddTrackEffect(const QDomElement &effect, int ix)
{
    ...
    if (effect.tagName() == "effectgroup") {
        ...
        for (int j = 0; j < effectlist.count(); ++j) {
            ...
            if (trackeffect.attribute("unique", "0") != "0" && m_timeline->hasTrackEffect(ix, trackeffect.attribute("tag"), trackeffect.attribute("id")) != -1) {
        ...
        if (effect.attribute("unique", "0") != "0" && m_timeline->hasTrackEffect(ix, effect.attribute("tag"), effect.attribute("id")) != -1) {
            emit displayMessage(i18n("Effect already present in track"), ErrorMessage);
...
void CustomTrackView::adjustEffects(ClipItem* item, ItemInfo oldInfo, QUndoCommand* command)
{
    ...
    if (!effects.isEmpty()) {
        ...
        while (i != effects.constEnd()) {
            new EditEffectCommand(this, item->track(), item->startPos(), i.value(), item->effect(i.key()), i.value().attribute("kdenlive_ix").toInt(), true, true, command);
            ++i;
        }
    }
}
