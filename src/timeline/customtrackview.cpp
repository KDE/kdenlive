...
bool CustomTrackView::event(QEvent *e)
{
    if (e->type() == QEvent::ShortcutOverride) {
        ...
        if ((mgr != nullptr) && mgr->trimMode() != NormalTrim) {
    ...
    if (m_tracksHeight != KdenliveSettings::trackheight()) {
        ...
        for (int i = 0; i < itemList.count(); ++i) {
            if (itemList.at(i)->type() == AVWidget) {
                ...
                item->setPos((qreal)item->startPos().frames(pCore->getCurrentFps()), getPositionFromTrack(item->track()) + 1);
                ...
                transitionitem->setPos((qreal)transitionitem->startPos().frames(pCore->getCurrentFps()),
                                       getPositionFromTrack(transitionitem->track()) + transitionitem->itemOffset());
...
int CustomTrackView::spaceToolSelectTrackOnly(int track, QList<QGraphicsItem *> &selection, GenTime pos)
{
    ...
    // qCDebug(KDENLIVE_LOG) << "SPACER TOOL + CTRL, SELECTING ALL CLIPS ON TRACK " << track << " WITH SELECTION RECT " << m_clickEvent.x() << '/' <<  track *
    // m_tracksHeight + 1 << "; " << mapFromScene(sceneRect().width(), 0).x() - m_clickEvent.x() << '/' << m_tracksHeight - 2;
    return 0;
}
...
GenTime CustomTrackView::createGroupForSelectedItems(QList<QGraphicsItem *> &selection)
{
    ...
    m_selectionGroup = new AbstractGroupItem(pCore->getCurrentFps());
...
void CustomTrackView::updateTimelineSelection()
{
    if (m_dragItem) {
        ...
        if (m_dragItem->type() == TransitionWidget && m_dragItem->isEnabled()) {
            ...
            if ((transitionClip != nullptr) && (transitionClip->binClip() != nullptr)) {
...
void CustomTrackView::mousePressEvent(QMouseEvent *event)
{
    ...
    if (event->button() == Qt::MidButton) {
        if (m_operationMode == KeyFrame) {
            if (m_dragItem->type() == AVWidget) {
                ...
                m_dragItem->insertKeyframe(item->getEffectAtIndex(item->selectedEffectIndex()), m_dragItem->selectedKeyFramePos(),
                                           -1, true);
                m_dragItem->update();
            }
        } else {
    ...
    double yOffset = 0;
    m_selectionMutex.lock();
    while ((m_dragGuide == nullptr) && ct < collisionList.count()) {
        if (collisionList.at(ct)->type() == AVWidget || collisionList.at(ct)->type() == TransitionWidget) {
            ...
            if ((m_dragItem->parentItem() != nullptr) && m_dragItem->parentItem()->type() == GroupWidget && m_dragItem->parentItem() != m_selectionGroup) {
                QGraphicsItem *topGroup = m_dragItem->parentItem();
                while ((topGroup->parentItem() != nullptr) && topGroup->parentItem()->type() == GroupWidget && topGroup->parentItem() != m_selectionGroup) {
    ...
    if (event->button() == Qt::RightButton) {
        ...
        if (!m_dragGuide) {
            ...
            QList<QGraphicsItem *> guidesCollisionList =
                items(event->pos().x() - 5, event->pos().y(), 10, 2); // a rect of height < 2 does not always collide with the guide
    ...
    ItemInfo clickInfo;
    clickInfo.startPos = GenTime(clickPoint.x(), pCore->getCurrentFps());
    ...
    if (m_dragItem) {
        ...
        if (m_dragItem->isSelected()) {
            itemSelected = true;
            selected = false;
        } else if ((m_dragItem->parentItem() != nullptr) && m_dragItem->parentItem()->isSelected()) {
            itemSelected = true;
        } else if ((dragGroup != nullptr) && dragGroup->isSelected()) {
            itemSelected = true;
        }
        ...
        if (!itemSelected) {
            ...
            if ((m_dragItem != nullptr) && m_dragItem->type() == AVWidget) {
    ...
    if (event->button() == Qt::LeftButton) {
        if (m_dragItem) {
            if ((m_selectionGroup != nullptr) && m_dragItem->parentItem() == m_selectionGroup) {
    ...
    if (m_operationMode == TransitionStart && event->modifiers() != Qt::ControlModifier) {
        ItemInfo info;
        ...
        if (transitiontrack != 0) {
            transitionClip = getClipItemAtMiddlePoint(info.startPos.frames(pCore->getCurrentFps()), transitiontrack);
        }
        if ((transitionClip != nullptr) && transitionClip->endPos() < m_dragItem->endPos()) {
            info.endPos = transitionClip->endPos();
        } else {
            GenTime transitionDuration(m_document->getFramePos(KdenliveSettings::transition_duration()), pCore->getCurrentFps());
        ...
        if (info.endPos == info.startPos) {
            info.endPos = info.startPos + GenTime(m_document->getFramePos(KdenliveSettings::transition_duration()), pCore->getCurrentFps());
        }
        ...
        for (int i = 0; i < selection.count(); ++i) {
            if (selection.at(i)->type() == TransitionWidget) {
                ...
                if (tr->startPos() - info.startPos > GenTime(5, pCore->getCurrentFps())) {
        ...
        ItemInfo info;
        info.endPos = GenTime(m_dragItem->endPos().frames(pCore->getCurrentFps()), pCore->getCurrentFps());
        info.track = m_dragItem->track();
        int transitiontrack = getPreviousVideoTrack(info.track);
        ClipItem *transitionClip = nullptr;
        if (transitiontrack != 0) {
            transitionClip = getClipItemAtMiddlePoint(info.endPos.frames(pCore->getCurrentFps()), transitiontrack);
        }
        if ((transitionClip != nullptr) && transitionClip->startPos() > m_dragItem->startPos()) {
        ...
        for (int i = 0; i < selection.count(); ++i) {
            if (selection.at(i)->type() == TransitionWidget) {
                Transition *tr = static_cast<Transition *>(selection.at(i));
                if (info.endPos - tr->endPos() > GenTime(5, pCore->getCurrentFps())) {
...
{
    const QPointF p((int)childPos.frames(pCore->getCurrentFps()), getPositionFromTrack(childTrack) + m_tracksHeight / 2);
...
GenTime CustomTrackView::groupSelectedItems(QList<QGraphicsItem *> selection, bool createNewGroup, bool selectNewGroup)
{
    ...
    for (int i = 0; i < selection.count(); ++i) {
        ...
        if (selection.at(i)->type() == GroupWidget) {
            ...
            while ((it->parentItem() != nullptr) && it->parentItem()->type() == GroupWidget) {
                it = static_cast<AbstractGroupItem *>(it->parentItem());
            }
            if ((it == nullptr) || it->isItemLocked()) {
                continue;
            }
    ...
    for (int i = 0; i < selection.count(); ++i) {
        if (selection.at(i)->type() == AVWidget || selection.at(i)->type() == TransitionWidget) {
            if ((selection.at(i)->parentItem() != nullptr) && selection.at(i)->parentItem()->type() == GroupWidget) {
...
void CustomTrackView::mouseDoubleClickEvent(QMouseEvent *event)
{
    ...
    if ((m_dragItem != nullptr) && m_dragItem->keyframesCount() > 0) {
        // add keyframe
        GenTime keyFramePos = GenTime((int)(mapToScene(event->pos()).x()), pCore->getCurrentFps()) - m_dragItem->startPos(); // + m_dragItem->cropStart();
        ...
        if (single == 1) {
            item->insertKeyframe(item->getEffectAtIndex(item->selectedEffectIndex()),
        ...
        item->insertKeyframe(item->getEffectAtIndex(item->selectedEffectIndex()), keyFramePos.frames(pCore->getCurrentFps()),
                             val);
        // QString next = item->keyframes(item->selectedEffectIndex());
        QDomElement newEffect = item->selectedEffect().cloneNode().toElement();
        EditEffectCommand *command =
            new EditEffectCommand(this, item->track(), item->startPos(), oldEffect, newEffect, item->selectedEffectIndex(), false, false, true);
        m_commandStack->push(command);
        updateEffect(item->track(), item->startPos(), item->selectedEffect());
        emit clipItemSelected(item, item->selectedEffectIndex() != 0);
    } else if ((m_dragItem != nullptr) && !m_dragItem->isItemLocked()) {
        editItemDuration();
    }
}
...
void CustomTrackView::editItemDuration()
{
    ...
    if (item->type() == GroupWidget || ((item->parentItem() != nullptr) && item->parentItem()->type() == GroupWidget)) {
    ...
    if (!item->isItemLocked()) {
        ...
        QPointer<ClipDurationDialog> d = new ClipDurationDialog(item, m_document->timecode(), minimum, maximum, this);
        if (d->exec() == QDialog::Accepted) {
            ...
            if (item->type() == TransitionWidget) {
                ...
                auto *command = new MoveTransitionCommand(this, startInfo, clipInfo, true, true);
...
void CustomTrackView::slotAttachKeyframeToEnd(bool attach)
{
    ...
    QDomElement newEffect = item->selectedEffect().cloneNode().toElement();
    EditEffectCommand *command =
        new EditEffectCommand(this, item->track(), item->startPos(), oldEffect, newEffect, item->selectedEffectIndex(), false, false, false);
    m_commandStack->push(command);
    updateEffect(item->track(), item->startPos(), item->selectedEffect());
    emit clipItemSelected(item, item->selectedEffectIndex() != 0);
}
    ...
    if (clip == nullptr || m_dragGuide) {
        ...
        if (clip->type() == AVWidget) {
            ...
            if (controller) {
                ...
                int offset = (item->startPos() - item->cropStart()).frames(pCore->getCurrentFps());
                if (!markers.isEmpty()) {
...
void CustomTrackView::insertClipCut(const QString &id, int in, int out)
{
    ...
    ItemInfo info;
    info.startPos = GenTime();
    info.cropStart = GenTime(in, pCore->getCurrentFps());
    info.endPos = GenTime(out - in, pCore->getCurrentFps());
    info.cropDuration = info.endPos - info.startPos;
    info.track = 0;

    // Check if clip can be inserted at that position
    ItemInfo pasteInfo = info;
    pasteInfo.startPos = GenTime(m_cursorPos, pCore->getCurrentFps());
    ...
    if (!ok) {
        ...
        int duration = GenTime(m_timeline->track(pasteInfo.track)->length()).frames(pCore->getCurrentFps());
        pasteInfo.startPos = GenTime(duration, pCore->getCurrentFps());
        pasteInfo.endPos = pasteInfo.startPos + info.endPos;
    ...
    auto *addCommand = new QUndoCommand();
...
bool CustomTrackView::insertDropClips(const QMimeData *mimeData, const QPoint &pos)
{
    ...
    if (mimeData->hasFormat(QStringLiteral("kdenlive/clip"))) {
        ...
        // Check if clip can be inserted at that position
        ItemInfo info;
        info.startPos = GenTime((int)(framePos.x() + 0.5), pCore->getCurrentFps());
        info.cropStart = GenTime(list.at(1).toInt(), pCore->getCurrentFps());
        info.cropDuration = GenTime(list.at(2).toInt() - list.at(1).toInt() + 1, pCore->getCurrentFps());
        ...
        m_selectionGroup = new AbstractGroupItem(pCore->getCurrentFps());
        auto *item = new ClipItem(clip, info, pCore->getCurrentFps(), 1.0, 1, getFrameWidth());
        ...
        GenTime start = GenTime((int)(framePos.x() + 0.5), pCore->getCurrentFps());
        framePos.setX((int)(framePos.x() + 0.5));
        framePos.setY(getPositionFromTrack(track));
        ...
        bool allowAudioOnly = false;
        for (int i = 0; i < ids.size(); ++i) {
            const QString &clipData = ids.at(i);
            ...
            if (clipData.contains(QLatin1Char('/'))) {
                ...
                info.cropStart = GenTime(in, pCore->getCurrentFps());
                info.cropDuration = GenTime(out - in + 1, pCore->getCurrentFps());
            } else {
        ...
        for (int i = 0; i < ids.size(); ++i) {
            ...
            auto *item = new ClipItem(clip, info, pCore->getCurrentFps(), 1.0, 1, getFrameWidth(), true);
            connect(item, &AbstractClipItem::selectItem, this, &CustomTrackView::slotSelectItem);
            item->setPos(info.startPos.frames(pCore->getCurrentFps()), item->itemOffset());
            if (ids.size() > 1) {
                m_selectionGroup->addItem(item);
            } else {
                m_dragItem = item;
...
{
    QRectF shape = QRectF(newPos.startPos.frames(pCore->getCurrentFps()), getPositionFromTrack(newPos.track) + 1,
                          (newPos.endPos - newPos.startPos).frames(pCore->getCurrentFps()) - 0.02, m_tracksHeight - 1);
...
void CustomTrackView::slotRefreshEffects(ClipItem *clip)
{
    ...
    for (int i = 0; i < clip->effectsCount(); ++i) {
        if (!m_timeline->track(track)->addEffect(pos.seconds(), EffectsController::getEffectArgs(clip->effect(i)))) {
            success = false;
        }
    }
    if (!success) {
...
void CustomTrackView::slotAddGroupEffect(const QDomElement &effect, AbstractGroupItem *group, AbstractClipItem *dropTarget)
{
    ...
    auto *effectCommand = new QUndoCommand();
    QString effectName;
...
bool CustomTrackView::createSplitOverlay(Mlt::Filter *filter)
{
    ...
    return m_timeline->createOverlay(filter, m_dragItem->track(), m_dragItem->startPos().frames(pCore->getCurrentFps()));
}
...
void CustomTrackView::processEffect(ClipItem *item, const QDomElement &effect, int offset, QUndoCommand *effectCommand)
{
    if (effect.attribute(QStringLiteral("type")) == QLatin1String("audio")) {
        ...
        if (item->clipState() == PlaylistState::VideoOnly || (item->clipType() != Audio && item->clipType() != AV && item->clipType() != Playlist)) {
            ...
            * and the message is annoying when working with the split audio feature */
            if ((item->parentItem() == nullptr) || item->parentItem() == m_selectionGroup) {
    ...
    if (effect.attribute(QStringLiteral("unique"), QStringLiteral("0")) != QLatin1String("0") &&
        item->hasEffect(effect.attribute(QStringLiteral("tag")), effect.attribute(QStringLiteral("id"))) != -1) {
    ...
    if (effect.attribute(QStringLiteral("id")) == QLatin1String("freeze") && m_cursorPos > item->startPos().frames(pCore->getCurrentFps()) &&
        m_cursorPos < item->endPos().frames(pCore->getCurrentFps())) {
        item->initEffect(effect, m_cursorPos - item->startPos().frames(pCore->getCurrentFps()), offset);
    } else {
...
{
    auto *delCommand = new QUndoCommand();
...
void CustomTrackView::slotDeleteEffect(ClipItem *clip, int track, const QDomElement &effect, bool affectGroup, QUndoCommand *parentCommand)
{
    ...
    if (affectGroup && (clip->parentItem() != nullptr) && clip->parentItem() == m_selectionGroup) {
...
void CustomTrackView::updateEffect(int track, GenTime pos, const QDomElement &insertedEffect, bool updateEffectStack, bool replaceEffect, bool refreshMonitor)
{
    ...
    if (pos < GenTime()) {
        ...
        EffectsParameterList effectParams = EffectsController::getEffectArgs(effect);
    ...
    if (clip) {
        ...
        if (effectId == QLatin1String("fadein") || effectId == QLatin1String("fade_from_black") || effectId == QLatin1String("fadeout") ||
    ...
    if (clip == nullptr) {
        ...
        for (int i = 0; i < effectIndexes.count(); ++i) {
            ...
            if (effect.attribute(QStringLiteral("id")) == QLatin1String("speed")) {
                ...
                EditEffectCommand *editcommand =
                    new EditEffectCommand(this, clip->track(), clip->startPos(), effect, newEffect, effectIndexes.at(i), false, true, true);
...
void CustomTrackView::slotUpdateClipEffect(ClipItem *clip, int track, const QDomElement &oldeffect, const QDomElement &effect, int ix, bool refreshEffectStack)
{
    EditEffectCommand *command;
    if (clip) {
        command = new EditEffectCommand(this, clip->track(), clip->startPos(), oldeffect.cloneNode().toElement(), effect.cloneNode().toElement(), ix,
                                        refreshEffectStack, true, true);
    } else {
...
void CustomTrackView::cutClip(const ItemInfo &info, const GenTime &cutTime, bool cut, const EffectsList &oldStack, bool execute)
{
    if (cut) {
        ...
        if ((item == nullptr) || !info.contains(cutTime)) {
        ...
        if (item) {
            ...
            if (item->checkKeyFrames(m_document->width(), m_document->height(), (info.cropDuration + info.cropStart).frames(pCore->getCurrentFps()))) {
                slotRefreshEffects(item);
            }
        }
        if (dup) {
            if (dup->checkKeyFrames(m_document->width(), m_document->height(), (info.cropDuration + info.cropStart).frames(pCore->getCurrentFps()),
                                    (cutTime - info.startPos).frames(pCore->getCurrentFps()))) {
    ...
    if ((item == nullptr) || (dup == nullptr) || item == dup) {
        emit displayMessage(i18n("Cannot find clip to uncut"), ErrorMessage);
        return;
    }
    if (!m_timeline->track(info.track)->del(cutTime.seconds())) {
        emit displayMessage(i18n("Error removing clip at %1 on track %2", m_document->timecode().getTimecodeFromFrames(cutTime.frames(pCore->getCurrentFps())),
    ...
    m_timeline->reloadTrack(info.track, info.startPos.frames(pCore->getCurrentFps()), info.endPos.frames(pCore->getCurrentFps()));
...
Transition *CustomTrackView::cutTransition(const ItemInfo &info, const GenTime &cutTime, bool cut, const QDomElement &oldStack, bool execute)
{
    if (cut) {
        ...
        if ((item == nullptr) || cutTime >= item->endPos() || cutTime <= item->startPos()) {
            emit displayMessage(i18n("Cannot find transition to cut"), ErrorMessage);
            if (item) {
                qCDebug(KDENLIVE_LOG) << "/////////  ERROR CUTTING transition : (" << item->startPos().frames(25) << '-' << item->endPos().frames(25)
                                      << "), INFO: (" << info.startPos.frames(25) << '-' << info.endPos.frames(25) << ')' << ", CUT: " << cutTime.frames(25);
            } else {
                qCDebug(KDENLIVE_LOG) << "/// ERROR NO transition at: " << info.startPos.frames(pCore->getCurrentFps()) << ", track: " << info.track;
            }
            return nullptr;
        }
        ...
        bool success = true;
        if (execute) {
            success = m_timeline->transitionHandler->moveTransition(item->transitionTag(), info.track, info.track, item->transitionEndTrack(), info.startPos,
                                                                    info.endPos, info.startPos, cutTime);
            if (success) {
                success = m_timeline->transitionHandler->addTransition(item->transitionTag(), item->transitionEndTrack(), info.track, cutTime, info.endPos,
                                                                       item->toXML());
            }
        }
        int cutPos = (int)cutTime.frames(pCore->getCurrentFps());
        ItemInfo newPos;
        ...
        dup->setPos(newPos.startPos.frames(pCore->getCurrentFps()), getPositionFromTrack(newPos.track) + 1 + dup->itemOffset());

        item->resizeEnd(cutPos);
        scene()->addItem(dup);

        if (item->checkKeyFrames(m_document->width(), m_document->height(), (info.cropDuration + info.cropStart).frames(pCore->getCurrentFps()))) {
            m_timeline->transitionHandler->updateTransitionParams(item->transitionTag(), item->transitionEndTrack(), info.track, info.startPos, cutTime,
                                                                  item->toXML());
        }
        if (dup->checkKeyFrames(m_document->width(), m_document->height(), (info.cropDuration + info.cropStart).frames(pCore->getCurrentFps()),
                                (cutTime - item->startPos()).frames(pCore->getCurrentFps()))) {
            m_timeline->transitionHandler->updateTransitionParams(item->transitionTag(), item->transitionEndTrack(), info.track, cutTime, info.endPos,
                                                                  dup->toXML());
        }
    ...
    ItemInfo clipinfo = item->info();
    bool success = m_timeline->transitionHandler->moveTransition(item->transitionTag(), clipinfo.track, clipinfo.track, item->transitionEndTrack(),
                                                                 clipinfo.startPos, clipinfo.endPos, clipinfo.startPos, transitionInfo.endPos);

    if (success) {
        item->resizeEnd((int)info.endPos.frames(pCore->getCurrentFps()));
        item->setTransitionParameters(oldStack);
        m_timeline->transitionHandler->updateTransitionParams(item->transitionTag(), item->transitionEndTrack(), info.track, info.startPos, info.endPos,
                                                              oldStack);
    } else {
...
void CustomTrackView::slotAddTransitionToSelectedClips(const QDomElement &transition, QList<QGraphicsItem *> itemList)
{
    ...
    if (itemList.count() == 1) {
        if (itemList.at(0)->type() == AVWidget) {
            ...
            GenTime pos = GenTime((int)(mapToScene(m_menuPosition).x()), pCore->getCurrentFps());
            if (pos < item->startPos() + item->cropDuration() / 2) {
                ...
                if ((transitionClip != nullptr) && transitionClip->endPos() < item->endPos()) {
                    info.endPos = transitionClip->endPos();
                } else {
                    info.endPos = info.startPos + GenTime(m_document->getFramePos(KdenliveSettings::transition_duration()), pCore->getCurrentFps());
                }
                ...
                if ((transitionClip != nullptr) && transitionClip->startPos() > item->startPos()) {
                    info.startPos = transitionClip->startPos();
                } else {
                    GenTime duration(m_document->getFramePos(KdenliveSettings::transition_duration()), pCore->getCurrentFps());
...
void CustomTrackView::slotAddTransition(ClipItem * /*clip*/, const ItemInfo &transitionInfo, int endTrack, const QDomElement &transition)
{
    ...
    auto *command = new AddTransitionCommand(this, transitionInfo, endTrack, transition, false, true);
    m_commandStack->push(command);
}
...
void CustomTrackView::addTransition(const ItemInfo &transitionInfo, int endTrack, const QDomElement &params, bool refresh)
{
    ...
    bool autotrans = params.attribute(QStringLiteral("automatic"), KdenliveSettings::automatictransitions() ? QStringLiteral("1") : QStringLiteral("0")) ==
                     QStringLiteral("1");
    auto *tr = new Transition(transitionInfo, endTrack, pCore->getCurrentFps(), params, autotrans);
    connect(tr, &AbstractClipItem::selectItem, this, &CustomTrackView::slotSelectItem);
    tr->setPos(transitionInfo.startPos.frames(pCore->getCurrentFps()), getPositionFromTrack(transitionInfo.track) + tr->itemOffset() + 1);
    ////qCDebug(KDENLIVE_LOG) << "---- ADDING transition " << params.attribute("value");
    if (m_timeline->transitionHandler->addTransition(tr->transitionTag(), endTrack, transitionInfo.track, transitionInfo.startPos, transitionInfo.endPos,
                                                     tr->toXML())) {
        scene()->addItem(tr);
        if (refresh) {
...
void CustomTrackView::deleteTransition(const ItemInfo &transitionInfo, int endTrack, const QDomElement & /*params*/, bool refresh)
{
    ...
    m_timeline->transitionHandler->deleteTransition(item->transitionTag(), endTrack, transitionInfo.track, transitionInfo.startPos, transitionInfo.endPos,
                                                    item->toXML(), refresh);
    if (m_dragItem == item) {
        m_dragItem->setMainSelectedClip(false);
        m_dragItem = nullptr;
...
void CustomTrackView::updateTransition(int track, const GenTime &pos, const QDomElement &oldTransition, const QDomElement &transition,
                                       bool updateTransitionWidget)
{
    Transition *item = getTransitionItemAt(pos, track);
    if (!item) {
        qCWarning(KDENLIVE_LOG) << "Unable to find transition at pos :" << pos.frames(pCore->getCurrentFps()) << ", ON track: " << track;
        return;
    }

    bool force = false;
    if (oldTransition.attribute(QStringLiteral("transition_atrack")) != transition.attribute(QStringLiteral("transition_atrack")) ||
        oldTransition.attribute(QStringLiteral("transition_btrack")) != transition.attribute(QStringLiteral("transition_btrack"))) {
        force = true;
    }
    m_timeline->transitionHandler->updateTransition(oldTransition.attribute(QStringLiteral("tag")), transition.attribute(QStringLiteral("tag")),
                                                    transition.attribute(QStringLiteral("transition_btrack")).toInt(),
                                                    transition.attribute(QStringLiteral("transition_atrack")).toInt(), item->startPos(), item->endPos(),
                                                    transition, force);
    ////qCDebug(KDENLIVE_LOG) << "ORIGINAL TRACK: "<< oldTransition.attribute("transition_btrack") << ", NEW TRACK:
...
{
    if (((m_selectionGroup != nullptr) || (m_dragItem != nullptr)) && m_clipDrag) {
...
void CustomTrackView::dropEvent(QDropEvent *event)
{
    ...
    if (((m_selectionGroup != nullptr) || (m_dragItem != nullptr)) && m_clipDrag) {
        ...
        if (m_selectionGroup) {
            items = m_selectionGroup->childItems();
            startPos = GenTime((int)m_selectionGroup->scenePos().x(), pCore->getCurrentFps());
        ...
        if (m_scene->editMode() == TimelineMode::InsertEdit) {
            cutTimeline(startPos.frames(pCore->getCurrentFps()), QList<ItemInfo>(), QList<ItemInfo>(), addCommand);
            ItemInfo info;
            ...
            /*for (int i = 0; i < items.count(); ++i) {
                ...
                extractZone(QPoint(info.startPos.frames(pCore->getCurrentFps()), (info.startPos+info.cropDuration).frames(pCore->getCurrentFps())), false,
...
void CustomTrackView::cutTimeline(int cutPos, const QList<ItemInfo> &excludedClips, const QList<ItemInfo> &excludedTransitions, QUndoCommand *masterCommand,
                                  int track)
{
    QRectF rect;
    if (track == -1) {
    ...
    for (int i = 0; i < selection.count(); ++i) {
        AbstractClipItem *item = static_cast<AbstractClipItem *>(selection.at(i));
        if ((item == nullptr) || !item->isEnabled() || (item->parentItem() == nullptr)) {
            continue;
        }
    ...
    // Razor
    GenTime cutPosition = GenTime(cutPos, pCore->getCurrentFps());
    for (int i = 0; i < selection.count(); ++i) {
...
void CustomTrackView::extractZone(QPoint z, bool closeGap, const QList<ItemInfo> &excludedClips, QUndoCommand *masterCommand, int track)
{
    ...
    GenTime inPoint(z.x(), pCore->getCurrentFps());
    GenTime outPoint(z.y(), pCore->getCurrentFps());
...
void CustomTrackView::adjustTimelineTransitions(TimelineMode::EditMode mode, Transition *item, QUndoCommand *command)
{
    if (mode == TimelineMode::OverwriteEdit) {
        ...
        KdenliveSettings::setSnaptopoints(false);
        ItemInfo info = item->info();
        QRectF rect(info.startPos.frames(pCore->getCurrentFps()), getPositionFromTrack(info.track) + m_tracksHeight,
                    (info.endPos - info.startPos).frames(pCore->getCurrentFps()) - 1, 5);
        QList<QGraphicsItem *> selection = m_scene->items(rect);
...
void CustomTrackView::removeTrack(int ix)
{
    ...
    if (m_timeline->getTrackInfo(ix).type == VideoTrack) {
        QScopedPointer<Mlt::Transition> tr(m_timeline->transitionHandler->getTrackTransition(
            QStringList() << QStringLiteral("qtblend") << QStringLiteral("frei0r.cairoblend") << QStringLiteral("movit.overlay"), ix, -1));
        if (tr) {
...
void CustomTrackView::lockTrack(int ix, bool lock, bool requestUpdate)
{
    ...
    for (int i = 0; i < selection.count(); ++i) {
        if (selection.at(i)->type() == GroupWidget && static_cast<AbstractGroupItem *>(selection.at(i)) != m_selectionGroup) {
            if ((selection.at(i)->parentItem() != nullptr) && (m_selectionGroup != nullptr)) {
    ...
    *ok = true;
    ...
    for (int i = 0; i < selection.count(); ++i) {
        ...
        if (selection.at(i)->type() == GroupWidget && (selection.at(i)->boundingRect().height() >= maxHeight)) {
            *ok = false;
            break;
        } else if ((selection.at(i)->parentItem() != nullptr) && (selection.at(i)->parentItem()->boundingRect().height() >= maxHeight)) {
            *ok = false;
            break;
        }
    }
...
void CustomTrackView::slotRemoveSpace(bool multiTrack)
{
    ...
    if (m_menuPosition.isNull()) {
        pos = GenTime(cursorPos(), pCore->getCurrentFps());
        ...
        pos = GenTime((int)(mapToScene(m_menuPosition).x()), pCore->getCurrentFps());
    ...
    ClipItem *item = getClipItemAtMiddlePoint(pos.frames(pCore->getCurrentFps()), track);
    if (item) {
        emit displayMessage(i18n("You must be in an empty space to remove space (time: %1, track: %2)",
                                 m_document->timecode().getTimecodeFromFrames(mapToScene(m_menuPosition).x()), track),
                            ErrorMessage);
        return;
    }
    ...
    if (multiTrack) {
        selection = selectAllItemsToTheRight(pos.frames(pCore->getCurrentFps()));
    } else {
    ...
    GenTime timeOffset(-length, pCore->getCurrentFps());
...
void CustomTrackView::slotInsertSpace()
{
    ...
    QPointer<SpacerDialog> d = new SpacerDialog(GenTime(65, pCore->getCurrentFps()), m_document->timecode(), track, m_timeline->getTracksInfo(), this);
    if (d->exec() != QDialog::Accepted) {
        delete d;
        return;
    }
    ...
    if (track >= 0) {
        ...
        if (item) {
            pos = item->startPos().frames(pCore->getCurrentFps());
        }

        if (spaceToolSelectTrackOnly(track, items, GenTime(pos, pCore->getCurrentFps())) == -1) {
            return;
        }
    } else {
...
{
    int pos = startPos.frames(pCore->getCurrentFps());
    QRectF rect;
    if (track == -1) {
        // all tracks
...
void CustomTrackView::insertSpace(const QList<ItemInfo> &clipsToMove, const QList<ItemInfo> &transToMove, int track, const GenTime &duration,
                                  const GenTime &offset)
{
    int diff = duration.frames(pCore->getCurrentFps());
    resetSelectionGroup();
    m_selectionMutex.lock();
    m_selectionGroup = new AbstractGroupItem(pCore->getCurrentFps());
    scene()->addItem(m_selectionGroup);
    ...
    if (!clipsToMove.isEmpty())
        for (int i = 0; i < clipsToMove.count(); ++i) {
            ...
            if (clip) {
                ...
                if (trackClipStartList.value(clipsToMove.at(i).track) == -1 ||
                    clipsToMove.at(i).startPos.frames(pCore->getCurrentFps()) < trackClipStartList.value(clipsToMove.at(i).track)) {
                    trackClipStartList[clipsToMove.at(i).track] = clipsToMove.at(i).startPos.frames(pCore->getCurrentFps());
                }
            } else {
                emit displayMessage(i18n("Cannot move clip at position %1, track %2",
                                         m_document->timecode().getTimecodeFromFrames((clipsToMove.at(i).startPos + offset).frames(pCore->getCurrentFps())),
                                         clipsToMove.at(i).track),
                                    ErrorMessage);
            }
        }
    if (!transToMove.isEmpty())
        for (int i = 0; i < transToMove.count(); ++i) {
            ...
            if (transition) {
                ...
                if (trackTransitionStartList.value(transToMove.at(i).track) == -1 ||
                    transToMove.at(i).startPos.frames(pCore->getCurrentFps()) < trackTransitionStartList.value(transToMove.at(i).track)) {
                    trackTransitionStartList[transToMove.at(i).track] = transToMove.at(i).startPos.frames(pCore->getCurrentFps());
                }
            } else {
                emit displayMessage(i18n("Cannot move transition at position %1, track %2",
                                         m_document->timecode().getTimecodeFromFrames(transToMove.at(i).startPos.frames(pCore->getCurrentFps())),
                                         transToMove.at(i).track),
                                    ErrorMessage);
            }
        }
        ...
        if (itemList.at(i)->type() == AVWidget) {
            ...
            if (item->getBinId() == clipId) {
                ...
                if (item->clipType() == Text) {
                    ...
                    if ((tr != nullptr) && tr->endPos() == item->endPos()) {
    ...
    if (timeOffset != GenTime()) {
        ...
        for (int i = 0; i < items.count(); ++i) {
            if (items.at(i)->type() == AVWidget) {
                ...
                if (trackClipStartList.value(info.track) == -1 || info.startPos.frames(pCore->getCurrentFps()) < trackClipStartList.value(info.track)) {
                    trackClipStartList[info.track] = info.startPos.frames(pCore->getCurrentFps());
                }
...
void CustomTrackView::mouseReleaseEvent(QMouseEvent *event)
{
    ...
    QPointF clickPoint = mapToScene(event->pos());
    GenTime clickFrame(clickPoint.x(), pCore->getCurrentFps());
...
void CustomTrackView::deleteSelectedClips()
{
    ...
    scene()->clearSelection();
    auto *deleteSelected = new QUndoCommand();
    ...
    for (int i = 0; i < itemList.count(); ++i) {
        ...
        } else if ((itemList.at(i)->parentItem() != nullptr) && itemList.at(i)->parentItem()->type() == GroupWidget) {
    ...
    for (int i = 0; i < itemList.count(); ++i) {
        if (itemList.at(i)->type() == AVWidget) {
            ...
            if (item->clipType() == Text) {
                Transition *tr = getTransitionItemAtStart(item->startPos(), item->track());
                if ((tr != nullptr) && !itemList.contains(tr) && tr->isAutomatic() && tr->endPos() == item->endPos()) {
...
void CustomTrackView::doChangeClipSpeed(const ItemInfo &info, const ItemInfo &speedIndependantInfo, PlaylistState::ClipState state, const double speed,
                                        int strobe, const QString &id, bool removeEffect)
{
    ClipItem *item = getClipItemAtStart(info.startPos, info.track);
    if (!item) {
    ...
    if (endPos >= 0) {
        ...
        if (item->cropDuration().frames(pCore->getCurrentFps()) != endPos - 1) {
            item->resizeEnd((int)info.startPos.frames(pCore->getCurrentFps()) + endPos);
        }
...
void CustomTrackView::cutSelectedClips(QList<QGraphicsItem *> itemList, GenTime currentPos)
{
    ...
    if (currentPos == GenTime()) {
        currentPos = GenTime(m_cursorPos, pCore->getCurrentFps());
    }
    if (itemList.isEmpty()) {
...
void CustomTrackView::razorGroup(AbstractGroupItem *group, GenTime cutPos)
{
    if (group) {
        ...
        if (clipsToCut.isEmpty() && transitionsCut.isEmpty() &&
            ((clips1.isEmpty() && transitions1.isEmpty()) || (clips2.isEmpty() && transitions2.isEmpty()))) {
            delete command;
            return;
        }
        // Process the cut
        for (int i = 0; i < clipsToCut.count(); ++i) {
            ...
            ItemInfo info = clip->info();
            info.endPos = GenTime(cutPos.frames(pCore->getCurrentFps()) - 1, pCore->getCurrentFps());
            info.cropDuration = info.endPos - info.startPos;
...
void CustomTrackView::groupClips(bool group, QList<QGraphicsItem *> itemList, bool forceLock, QUndoCommand *command, bool doIt)
{
    ...
    for (int i = 0; i < max; ++i) {
        ...
        } else if ((item->parentItem() != nullptr) && item->parentItem()->type() == GroupWidget) {
    ...
    if (!clipInfos.isEmpty()) {
        ...
        if (group && (command == nullptr) && !existingGroups.isEmpty()) {
        ...
        if (command) {
            ...
            if (metaCommand) {
                ...
                auto *groupCommand = new GroupClipsCommand(this, clipInfos, transitionInfos, group, doIt);
...
void CustomTrackView::doGroupClips(const QList<ItemInfo> &clipInfos, const QList<ItemInfo> &transitionInfos, bool group)
{
    ...
    if (!group) {
        ...
        for (int i = 0; i < transitionInfos.count(); ++i) {
            ...
            if ((tr->parentItem() != nullptr) && tr->parentItem()->type() == GroupWidget) {
    ...
    for (int i = 0; i < clipInfos.count(); ++i) {
        ClipItem *clip = getClipItemAtStart(clipInfos.at(i).startPos, clipInfos.at(i).track);
        if ((clip != nullptr) && !list.contains(clip)) {
            list.append(clip);
            // clip->setSelected(true);
        }
    }
...
void CustomTrackView::addClip(const QString &clipId, const ItemInfo &info, const EffectsList &effects, PlaylistState::ClipState state, bool refresh)
{
    ...
    auto *item = new ClipItem(binClip, info, pCore->getCurrentFps(), speed, strobe, getFrameWidth());
    connect(item, &AbstractClipItem::selectItem, this, &CustomTrackView::slotSelectItem);
    item->setPos(info.startPos.frames(pCore->getCurrentFps()), getPositionFromTrack(info.track) + 1 + item->itemOffset());
    item->setState(state);
    ...
    binClip->addRef();
    m_timeline->track(info.track)
        ->add(info.startPos.seconds(), prod, info.cropStart.seconds(), (info.cropStart + info.cropDuration).seconds(), state, duplicate,
              TimelineMode::NormalEdit); // m_scene->editMode());

    for (int i = 0; i < item->effectsCount(); ++i) {
        m_timeline->track(info.track)->addEffect(info.startPos.seconds(), EffectsController::getEffectArgs(item->effect(i)));
    }
    if (refresh && item->hasVisibleVideo()) {
        monitorRefresh(info, true);
    }
...
{
    int framepos = (int)(pos.frames(pCore->getCurrentFps()));
...
{
    QList<QGraphicsItem *> list = scene()->items(QPointF(pos.frames(pCore->getCurrentFps()), getPositionFromTrack(track) + m_tracksHeight / 2));
    ClipItem *clip = nullptr;
    for (int i = 0; i < list.size(); ++i) {
        if (!list.at(i)->isEnabled()) {
            /*ClipItem *test = static_cast <ClipItem *>(list.at(i));
...
{
    QList<QGraphicsItem *> list =
        scene()->items(QPointF((info.startPos + offset).frames(pCore->getCurrentFps()), getPositionFromTrack(info.track + trackOffset) + m_tracksHeight / 2));
    ClipItem *clip = nullptr;
    for (int i = 0; i < list.size(); ++i) {
...
{
    return getTransitionItemAt(pos.frames(pCore->getCurrentFps()), track, alreadyMoved);
}
...
{
    const QPointF p(pos.frames(pCore->getCurrentFps()), getPositionFromTrack(track) + Transition::itemOffset() + 1);
    QList<QGraphicsItem *> list = scene()->items(p);
    Transition *clip = nullptr;
    for (int i = 0; i < list.size(); ++i) {
        if (!list.at(i)->isEnabled()) {
...
bool CustomTrackView::moveClip(const ItemInfo &start, const ItemInfo &end, bool refresh, bool alreadyMoved, ItemInfo *out_actualEnd)
{
    ...
    if (!item) {
        emit displayMessage(i18n("Cannot move clip at time: %1 on track %2",
                                 m_document->timecode().getTimecodeFromFrames(start.startPos.frames(pCore->getCurrentFps())), start.track),
                            ErrorMessage);
    ...
    if (!success) {
        ...
        emit displayMessage(i18n("Cannot move clip to position %1", m_document->timecode().getTimecodeFromFrames(end.startPos.frames(pCore->getCurrentFps()))),
                            ErrorMessage);
    } else if (!alreadyMoved) {
        bool snap = KdenliveSettings::snaptopoints();
        KdenliveSettings::setSnaptopoints(false);
        item->setPos((int)end.startPos.frames(pCore->getCurrentFps()), getPositionFromTrack(end.track) + 1);
        ...
        if (item->baseClip()->isTransparent()) {
            ...
            if (tr && tr->isAutomatic()) {
                ...
                m_document->renderer()->mltMoveTransition(tr->transitionTag(), m_timeline->tracksCount() - start.track, m_timeline->tracksCount() - end.track,
        tr->transitionEndTrack(), start.startPos, start.endPos, end.startPos, end.endPos);
                tr->setPos((int) end.startPos.frames(pCore->getCurrentFps()), (int)(end.track * m_tracksHeight + 1));
            }
        }*/
        KdenliveSettings::setSnaptopoints(snap);
    }
...
void CustomTrackView::moveGroup(QList<ItemInfo> startClip, QList<ItemInfo> startTransition, const GenTime &offset, const int trackOffset, bool alreadyMoved,
                                bool reverseMove)
{
    // Group Items
    resetSelectionGroup();
    ...
    for (int i = 0; i < startClip.count(); ++i) {
        ...
        if (clip) {
            ...
            qCDebug(KDENLIVE_LOG) << "//MISSING CLIP AT: " << startClip.at(i).startPos.frames(25) << " / track: " << startClip.at(i).track
                                  << " / OFFSET: " << trackOffset;
        }
    }
    for (int i = 0; i < startTransition.count(); ++i) {
        ...
        if (tr) {
            ...
            m_timeline->transitionHandler->deleteTransition(tr->transitionTag(), tr->transitionEndTrack(), startTransition.at(i).track,
                                                            startTransition.at(i).startPos, startTransition.at(i).endPos, tr->toXML());
        } else {
    ...
    if (m_selectionGroup) {
        ...
        if (!alreadyMoved) {
            m_selectionGroup->setTransform(QTransform::fromTranslate(offset.frames(pCore->getCurrentFps()), -trackOffset * (qreal)m_tracksHeight), true);
        }
        ...
        for (int i = 0; i < max; ++i) {
            if (children.at(i)->type() == GroupWidget) {
                ...
                // grp->moveBy(offset.frames(pCore->getCurrentFps()), trackOffset *(qreal) m_tracksHeight);
                /*m_document->clipManager()->removeGroup(grp);
        ...
        for (int i = 0; i < children.count(); ++i) {
            ...
            if (item->type() == AVWidget) {
                ...
                m_timeline->track(info.track)
                    ->add(info.startPos.seconds(), prod, info.cropStart.seconds(), (info.cropStart + info.cropDuration).seconds(), clip->clipState(), true,
                          m_scene->editMode());

                for (int j = 0; j < clip->effectsCount(); ++j) {
                    m_timeline->track(info.track)
                        ->addEffect(info.startPos.seconds(), EffectsController::getEffectArgs(clip->effect(j)));
                }
            } else if (item->type() == TransitionWidget) {
...
void CustomTrackView::moveTransition(const ItemInfo &start, const ItemInfo &end, bool refresh)
{
    ...
    if (!item) {
        emit displayMessage(i18n("Cannot move transition at time: %1 on track %2",
                                 m_document->timecode().getTimecodeFromFrames(start.startPos.frames(pCore->getCurrentFps())), start.track),
                            ErrorMessage);
        // qCDebug(KDENLIVE_LOG) << "----------------  ERROR, CANNOT find transition to move... ";// << startPos.x() * m_scale * FRAME_SIZE + 1 << ", " <<
        // startPos.y() * m_tracksHeight + m_tracksHeight / 2;
        return;
    }
    ...
    if (end.endPos - end.startPos == start.endPos - start.startPos) {
        ...
        item->resizeStart((int)end.startPos.frames(pCore->getCurrentFps()));
    } else if (end.startPos == start.startPos) {
        // Transition end resize;
        // qCDebug(KDENLIVE_LOG) << "// resize END: " << end.endPos.frames(pCore->getCurrentFps());
        item->resizeEnd((int)end.endPos.frames(pCore->getCurrentFps()));
    } else {
        // Move & resize
        item->setPos((int)end.startPos.frames(pCore->getCurrentFps()), getPositionFromTrack(end.track) + 1);
        item->resizeStart((int)end.startPos.frames(pCore->getCurrentFps()));
    ...
    // item->transitionHandler->moveTransition(GenTime((int) (endPos.x() - startPos.x()), pCore->getCurrentFps()));
    KdenliveSettings::setSnaptopoints(snap);
    item->updateTransitionEndTrack(getPreviousVideoTrack(end.track));
    m_timeline->transitionHandler->moveTransition(item->transitionTag(), start.track, item->track(), item->transitionEndTrack(), start.startPos, start.endPos,
                                                  item->startPos(), item->endPos());
    if ((m_dragItem != nullptr) && m_dragItem == item) {
        QPoint p;
...
{
    if (pos == oldInfo.startPos.frames(pCore->getCurrentFps())) {
        return;
    }
    bool snap = KdenliveSettings::snaptopoints();
    if (check) {
        ...
        if (item->startPos().frames(pCore->getCurrentFps()) != pos) {
            item->resizeStart(oldInfo.startPos.frames(pCore->getCurrentFps()));
    ...
    if ((item->parentItem() != nullptr) && item->parentItem() != m_selectionGroup) {
        new RebuildGroupCommand(this, item->info().track, item->endPos() - GenTime(1, pCore->getCurrentFps()), command);
    }
    ItemInfo info = item->info();
    if (item->type() == AVWidget) {
        ...
        if (success) {
            ...
            if ((transition != nullptr) && transition->isAutomatic()) {
                ...
                if (newTrInfo.startPos < newTrInfo.endPos) {
                    ...
                    if (transition->updateKeyframes(trInfo, newTrInfo)) {
                        ...
                        m_timeline->transitionHandler->updateTransition(xml.attribute(QStringLiteral("tag")), xml.attribute(QStringLiteral("tag")),
            ...
            if ((transition != nullptr) && transition->isAutomatic() && transition->transitionEndTrack() == oldInfo.track) {
                ItemInfo trInfo = transition->info();
                ItemInfo newTrInfo = trInfo;
                newTrInfo.startPos = item->startPos();
                newTrInfo.cropDuration = trInfo.endPos - newTrInfo.startPos;
                ...
                // TODO
                if ((upperClip == nullptr /*|| !upperClip->baseClip()->isTransparent()*/) && newTrInfo.startPos < newTrInfo.endPos) {
                    QDomElement old = transition->toXML();
                    if (transition->updateKeyframes(trInfo, newTrInfo)) {
                        QDomElement xml = transition->toXML();
                        m_timeline->transitionHandler->updateTransition(xml.attribute(QStringLiteral("tag")), xml.attribute(QStringLiteral("tag")),
            ...
            KdenliveSettings::setSnaptopoints(false);
            item->resizeStart((int)oldInfo.startPos.frames(pCore->getCurrentFps()));
            KdenliveSettings::setSnaptopoints(snap);
            emit displayMessage(i18n("Error when resizing clip"), ErrorMessage);
        }
    } else if (item->type() == TransitionWidget) {
        Transition *transition = static_cast<Transition *>(item);
        if (!m_timeline->transitionHandler->moveTransition(transition->transitionTag(), oldInfo.track, oldInfo.track, transition->transitionEndTrack(),
                                                           oldInfo.startPos, oldInfo.endPos, info.startPos, info.endPos)) {
            // Cannot resize transition
            KdenliveSettings::setSnaptopoints(false);
            transition->resizeStart((int)oldInfo.startPos.frames(pCore->getCurrentFps()));
            KdenliveSettings::setSnaptopoints(snap);
            ...
            if (transition->updateKeyframes(oldInfo, info)) {
                ...
                m_timeline->transitionHandler->updateTransition(xml.attribute(QStringLiteral("tag")), xml.attribute(QStringLiteral("tag")),
                                                                xml.attribute(QStringLiteral("transition_btrack")).toInt(),
                                                                xml.attribute(QStringLiteral("transition_atrack")).toInt(), info.startPos, info.endPos, xml);
                new EditTransitionCommand(this, transition->track(), transition->startPos(), old, xml, false, command);
            }
...
{
    if (pos == oldInfo.endPos.frames(pCore->getCurrentFps())) {
        return;
    }
    bool snap = KdenliveSettings::snaptopoints();
    if (check) {
        KdenliveSettings::setSnaptopoints(false);
        item->resizeEnd(pos);
        if (item->endPos().frames(pCore->getCurrentFps()) != pos) {
            item->resizeEnd(oldInfo.endPos.frames(pCore->getCurrentFps()));
            emit displayMessage(i18n("Not possible to resize"), ErrorMessage);
            KdenliveSettings::setSnaptopoints(snap);
            return;
        }
    ...
    if (item->type() == AVWidget) {
        ...
        if (success) {
            ...
            if ((tr != nullptr) && tr->isAutomatic() && tr->transitionEndTrack() == oldInfo.track) {
                ItemInfo trInfo = tr->info();
                ItemInfo newTrInfo = trInfo;
                newTrInfo.endPos = item->endPos();
                newTrInfo.cropDuration = newTrInfo.endPos - trInfo.startPos;
                ...
                // TODO
                if ((upperClip == nullptr /*|| !upperClip->baseClip()->isTransparent()*/) && newTrInfo.endPos > newTrInfo.startPos) {
                    QDomElement old = tr->toXML();
                    if (tr->updateKeyframes(trInfo, newTrInfo)) {
                        QDomElement xml = tr->toXML();
                        m_timeline->transitionHandler->updateTransition(xml.attribute(QStringLiteral("tag")), xml.attribute(QStringLiteral("tag")),
            ...
            KdenliveSettings::setSnaptopoints(false);
            item->resizeEnd((int)oldInfo.endPos.frames(pCore->getCurrentFps()));
        ...
        if (!m_timeline->transitionHandler->moveTransition(transition->transitionTag(), oldInfo.track, oldInfo.track, transition->transitionEndTrack(),
            ...
            // Cannot resize transition
            KdenliveSettings::setSnaptopoints(false);
            transition->resizeEnd((int)oldInfo.endPos.frames(pCore->getCurrentFps()));
            KdenliveSettings::setSnaptopoints(true);
            emit displayMessage(i18n("Cannot resize transition"), ErrorMessage);
        } else {
            ...
            if (transition->updateKeyframes(oldInfo, newInfo)) {
                QDomElement xml = transition->toXML();
                m_timeline->transitionHandler->updateTransition(
                    xml.attribute(QStringLiteral("tag")), xml.attribute(QStringLiteral("tag")), xml.attribute(QStringLiteral("transition_btrack")).toInt(),
                    xml.attribute(QStringLiteral("transition_atrack")).toInt(), transition->startPos(), transition->endPos(), xml);
                new EditTransitionCommand(this, transition->track(), transition->startPos(), old, xml, false, command);
            }
...
void CustomTrackView::updatePositionEffects(ClipItem *item, const ItemInfo &info, bool standalone)
{
    ...
    if (effectPos != -1) {
        ...
        if (!m_timeline->track(item->track())
                 ->editEffect(item->startPos().seconds(), EffectsController::getEffectArgs(effect), false)) {
            emit displayMessage(i18n("Problem editing effect"), ErrorMessage);
        }
    ...
    if (effectPos != -1) {
        ...
        QDomElement effect = item->getEffectAtIndex(effectPos);
        int max = item->cropDuration().frames(pCore->getCurrentFps());
        int end = max + item->cropStart().frames(pCore->getCurrentFps()) - 1;
        int duration = EffectsList::parameter(effect, QStringLiteral("out")).toInt() - EffectsList::parameter(effect, QStringLiteral("in")).toInt();
        if (duration > max) {
            // Make sure the fade effect is not longer than the clip
            duration = max;
    ...
    if (effectPos != -1) {
        ...
        int diff = (info.startPos - item->startPos()).frames(pCore->getCurrentFps());
...
void CustomTrackView::updateSnapPoints(AbstractClipItem *selected, const QList<GenTime> &offsetList, bool skipSelectedItems)
{
    /*
    QList<GenTime> snaps;
    ...
    // add cursor position
    GenTime pos = GenTime(m_cursorPos, pCore->getCurrentFps());
    ...
    pos = GenTime(z.x(), pCore->getCurrentFps());
    if (!snaps.contains(pos)) {
        snaps.append(pos);
    }
    pos = GenTime(z.y(), pCore->getCurrentFps());
    if (!snaps.contains(pos)) {
        snaps.append(pos);
    }
...
{
    updateSnapPoints(nullptr);
    seekCursorPos((int)m_scene->previousSnapPoint(GenTime(m_cursorPos, pCore->getCurrentFps())).frames(pCore->getCurrentFps()));
    checkScrolling();
}
...
{
    updateSnapPoints(nullptr);
    seekCursorPos((int)m_scene->nextSnapPoint(GenTime(m_cursorPos, pCore->getCurrentFps())).frames(pCore->getCurrentFps()));
    checkScrolling();
}
...
void CustomTrackView::clipStart()
{
    ...
    if (item != nullptr) {
        seekCursorPos((int)item->startPos().frames(pCore->getCurrentFps()));
        checkScrolling();
    }
}
...
void CustomTrackView::clipEnd()
{
    ...
    if (item != nullptr) {
        seekCursorPos((int)item->endPos().frames(pCore->getCurrentFps()) - 1);
        checkScrolling();
    }
}
...
{
    for (int i = 0; i < m_guides.count(); ++i) {
        double guidePos = m_guides.at(i)->position().frames(pCore->getCurrentFps()) * (framePos ? 1 : matrix().m11());
        if (framePos) {
            if (guidePos == pos) {
                return guidePos;
            }
...
void CustomTrackView::buildGuidesMenu(QMenu *goMenu) const
{
    ...
    for (int i = 0; i < m_guides.count(); ++i) {
        ...
        act->setData(m_guides.at(i)->position().frames(pCore->getCurrentFps()));
    }
...
bool CustomTrackView::addGuide(const GenTime &pos, const QString &comment, bool loadingProject)
{
    for (int i = 0; i < m_guides.count(); ++i) {
        if (m_guides.at(i)->position() == pos) {
            emit displayMessage(i18n("A guide already exists at position %1", m_document->timecode().getTimecodeFromFrames(pos.frames(pCore->getCurrentFps()))),
                                ErrorMessage);
            return false;
        }
    }
    auto *g = new Guide(this, pos, comment, m_tracksHeight * m_timeline->visibleTracksCount() * matrix().m22());
    scene()->addItem(g);
    m_guides.append(g);
    qSort(m_guides.begin(), m_guides.end(), sortGuidesList);
    if (!loadingProject) {
...
void CustomTrackView::slotEditGuide(int guidePos, const QString &newText)
{
    ...
    if (guidePos == -1) {
        ...
        pos = GenTime(guidePos, pCore->getCurrentFps());
    }
    bool found = false;
    for (int i = 0; i < m_guides.count(); ++i) {
        if (m_guides.at(i)->position() == pos) {
...
{
    auto *deleteAll = new QUndoCommand();
...
bool CustomTrackView::findString(const QString &text)
{
    ...
    for (int i = 0; i < m_searchPoints.size(); ++i) {
        ...
        if (marker.contains(text, Qt::CaseInsensitive)) {
            seekCursorPos(m_searchPoints.at(i).time().frames(pCore->getCurrentFps()));
...
bool CustomTrackView::canBePastedTo(const ItemInfo &info, int type, const QList<AbstractClipItem *> &excluded) const
{
    ...
    QRectF rect((double)info.startPos.frames(pCore->getCurrentFps()), (double)(getPositionFromTrack(info.track) + 1 + offset),
                (double)(info.endPos - info.startPos).frames(pCore->getCurrentFps()), (double)height);
...
bool CustomTrackView::canBePastedTo(const QList<ItemInfo> &infoList, int type) const
{
    ...
    for (int i = 0; i < infoList.count(); ++i) {
        const QRectF rect((double)infoList.at(i).startPos.frames(pCore->getCurrentFps()), (double)(getPositionFromTrack(infoList.at(i).track) + 1),
                          (double)(infoList.at(i).endPos - infoList.at(i).startPos).frames(pCore->getCurrentFps()), (double)(m_tracksHeight - 1));
        path.addRect(rect);
    }
...
void CustomTrackView::pasteClip()
{
    ...
    if (pos == GenTime(-1)) {
        pos = GenTime((int)(mapToScene(position).x()), pCore->getCurrentFps());
    }
    if (track == -1) {
    ...
    auto *pasteClips = new QUndoCommand();
    ...
    for (int i = 0; i < m_copiedItems.count(); ++i) {
        // parse all clip names
        if ((m_copiedItems.at(i) != nullptr) && m_copiedItems.at(i)->type() == AVWidget) {
        ...
        } else if ((m_copiedItems.at(i) != nullptr) && m_copiedItems.at(i)->type() == TransitionWidget) {
    ...
    for (int i = 0; i < clips.count(); ++i) {
        if (clips.at(i)->type() == AVWidget) {
            ...
            for (int j = 0; j < clip->effectsCount(); ++j) {
                ...
                if (eff.attribute(QStringLiteral("unique"), QStringLiteral("0")) == QLatin1String("0") ||
                    item->hasEffect(eff.attribute(QStringLiteral("tag")), eff.attribute(QStringLiteral("id"))) == -1) {
    ...
    for (int i = 0; i < params.count(); ++i) {
        ...
        if (e.attribute(QStringLiteral("type")) == QLatin1String("keyframe") || e.attribute(QStringLiteral("type")) == QLatin1String("simplekeyframe")) {
            ...
            QString adjusted = EffectsController::adjustKeyframes(e.attribute(QStringLiteral("keyframes")), oldstart.frames(pCore->getCurrentFps()),
                                                                  ...
                                                                  (newstart + duration).frames(pCore->getCurrentFps()) - 1);
            ...
            if (xml.attribute(QStringLiteral("kdenlive:sync_in_out")) != QLatin1String("1")) {
                ...
                xml.setAttribute(QStringLiteral("in"), QString::number(newstart.frames(pCore->getCurrentFps())));
                xml.setAttribute(QStringLiteral("out"), QString::number((newstart + duration).frames(pCore->getCurrentFps())));
            }
        }
    }
}
...
ClipItem *CustomTrackView::getClipUnderCursor() const
{
    ...
    QList<QGraphicsItem *> collisions = scene()->items(rect, Qt::IntersectsItemBoundingRect);
    if ((m_dragItem != nullptr) && !collisions.isEmpty() && collisions.contains(m_dragItem) && m_dragItem->type() == AVWidget && !m_dragItem->isItemLocked()) {
...
const QString CustomTrackView::getClipUnderCursor(int *pos, QPoint *zone) const
{
    ...
    if (item == nullptr) {
        ...
        *pos = seekPosition() - (item->startPos() - item->cropStart()).frames(pCore->getCurrentFps());
        if (zone) {
            *zone = QPoint(item->cropStart().frames(pCore->getCurrentFps()), (item->cropStart() + item->cropDuration()).frames(pCore->getCurrentFps()));
        }
    }
    QString result;
    if (item) {
...
AbstractClipItem *CustomTrackView::getMainActiveClip() const
{
    ...
    for (int i = 0; i < clips.count(); ++i) {
        if (clips.at(i)->type() == AVWidget) {
            ...
            if (clips.count() > 1 && item->startPos().frames(pCore->getCurrentFps()) <= seekPosition() &&
                item->endPos().frames(pCore->getCurrentFps()) >= seekPosition()) {
                break;
            }
        }
    }
...
ClipItem *CustomTrackView::getActiveClipUnderCursor(bool allowOutsideCursor) const
{
    ...
    for (int i = 0; i < clips.count(); ++i) {
        if (clips.at(i)->type() == AVWidget) {
            ...
            if (item->startPos().frames(pCore->getCurrentFps()) <= seekPosition() && item->endPos().frames(pCore->getCurrentFps()) >= seekPosition()) {
                return item;
            }
        }
    }
...
void CustomTrackView::expandActiveClip()
{
    ...
    auto *expandCommand = new QUndoCommand();
...
void CustomTrackView::setInPoint()
{
    ...
    if (clip == nullptr) {
        if ((m_dragItem != nullptr) && m_dragItem->type() == TransitionWidget) {
            clip = m_dragItem;
        } else {
            emit displayMessage(i18n("You must select one clip for this action"), ErrorMessage);
            return;
...
void CustomTrackView::slotUpdateAllThumbs()
{
    ...
    for (int i = 0; i < itemList.count(); ++i) {
        if (itemList.at(i)->type() == AVWidget) {
            item = static_cast<ClipItem *>(itemList.at(i));
            if ((item != nullptr) && item->isEnabled() && item->clipType() != Color && item->clipType() != Audio) {
                ...
                if (item->clipType() == Image || item->clipType() == Text) {
                    ...
                    startThumb.append(QString::number((int)item->speedIndependantCropStart().frames(pCore->getCurrentFps())) + QStringLiteral(".png"));
                    endThumb.append(
                        QString::number((int)(item->speedIndependantCropStart() + item->speedIndependantCropDuration()).frames(pCore->getCurrentFps()) - 1) +
                        QStringLiteral(".png"));
...
{
    QPointer<TracksConfigDialog> d = new TracksConfigDialog(m_timeline, ix, parentWidget());
    if (d->exec() == QDialog::Accepted) {
...
void CustomTrackView::deleteTimelineTrack(int ix, const TrackInfo &trackinfo)
{
    ...
    QList<QGraphicsItem *> selection = m_scene->items(r);
    auto *deleteTrack = new QUndoCommand();
    ...
    for (int i = 0; i < groupsToProceed.count(); i++) {
        ...
        if (!clipGrpInfos.isEmpty() || !transitionGrpInfos.isEmpty()) {
            // Break group
            new GroupClipsCommand(this, clipGrpInfos, transitionGrpInfos, false, deleteTrack != nullptr);
...
void CustomTrackView::autoTransition()
{
    ...
    m_timeline->transitionHandler->updateTransition(transition.attribute(QStringLiteral("tag")), transition.attribute(QStringLiteral("tag")),
                                                    transition.attribute(QStringLiteral("transition_btrack")).toInt(),
                                                    transition.attribute(QStringLiteral("transition_atrack")).toInt(), tr->startPos(), tr->endPos(),
                                                    transition);
    setDocumentModified();
}
...
void CustomTrackView::getClipAvailableSpace(AbstractClipItem *item, GenTime &minimum, GenTime &maximum)
{
    ...
    for (int i = 0; i < selection.count(); ++i) {
        ...
        if ((clip != nullptr) && clip->type() == AVWidget) {
...
void CustomTrackView::getTransitionAvailableSpace(AbstractClipItem *item, GenTime &minimum, GenTime &maximum)
{
    ...
    for (int i = 0; i < selection.count(); ++i) {
        AbstractClipItem *clip = static_cast<AbstractClipItem *>(selection.at(i));
        if ((clip != nullptr) && clip->type() == TransitionWidget) {
            if (clip->endPos() <= item->startPos() && clip->endPos() > minimum) {
                minimum = clip->endPos();
            }
            if (clip->startPos() > item->startPos() && (clip->startPos() < maximum || maximum == GenTime())) {
    ...
    for (int i = 0; i < groups.count(); ++i) {
        ...
        for (int nodeindex = 0; nodeindex < children.count(); ++nodeindex) {
            ...
            if (elem.tagName() == QLatin1String("clipitem")) {
                ClipItem *clip = getClipItemAtStart(GenTime(pos, pCore->getCurrentFps()), track);
                if (clip) {
    ...
    for (QGraphicsItem *item : selection) {
        if (item->type() == AVWidget) {
            ...
            if (clip->clipType() == AV || clip->clipType() == Audio) {
                ItemInfo info = clip->info();
                Mlt::Producer *prod =
                    m_timeline->track(clip->track())->clipProducer(m_document->renderer()->getBinProducer(clip->getBinId()), clip->clipState());
                if (!prod) {
                    qCWarning(KDENLIVE_LOG) << "couldn't load producer for clip " << clip->getBinId() << " on track " << clip->track();
                    return;
                }
...
void CustomTrackView::slotAlignClip(int track, int pos, int shift)
{
    ...
    GenTime add(shift, pCore->getCurrentFps());
...
bool CustomTrackView::doSplitAudio(const GenTime &pos, int track, int destTrack, bool split)
{
    ...
    if (split) {
        int start = pos.frames(pCore->getCurrentFps());
        ...
        if (destTrack == -1) {
            ...
            for (; destTrack > 0; destTrack--) {
                ...
                if (info.type == AudioTrack && !info.isLocked) {
                    ...
                    if (blength == -1 || blength >= clip->cropDuration().frames(pCore->getCurrentFps())) {
                        break;
                    }
                }
            }
            ...
            if (info.type != AudioTrack || info.isLocked) {
                ...
                int blength = m_timeline->getTrackSpaceLength(destTrack, start, false);
                if (blength >= 0 && blength < clip->cropDuration().frames(pCore->getCurrentFps())) {
                    destTrack = 0;
                }
            }
        }
        ...
        if (audioClip) {
            if (m_timeline->track(track)->replace(pos.seconds(), m_document->renderer()->getBinVideoProducer(clip->getBinId()))) {
                ...
                emit displayMessage(i18n("Cannot update clip (time: %1, track: %2)", pos.frames(pCore->getCurrentFps()), destTrack), ErrorMessage);
            }
...
void CustomTrackView::setClipType(PlaylistState::ClipState state)
{
    ...
    auto *videoCommand = new QUndoCommand();
...
void CustomTrackView::monitorRefresh(const QList<ItemInfo> &range, bool invalidateRange)
{
    ...
    for (int i = 0; i < range.count(); i++) {
        if (range.at(i).contains(GenTime(m_cursorPos, pCore->getCurrentFps()))) {
            refreshMonitor = true;
        }
        if (invalidateRange) {
...
{
    if (range.contains(GenTime(m_cursorPos, pCore->getCurrentFps()))) {
        m_document->renderer()->doRefresh();
    }
    if (invalidateRange) {
...
void CustomTrackView::doChangeClipType(const ItemInfo &info, PlaylistState::ClipState state)
{
    ...
    if ((prod != nullptr) && prod->is_valid() && m_timeline->track(info.track)->replace(info.startPos.seconds(), prod, state, previousState)) {
        clip->setState(state);
        clip->update();
        if (clip->clipType() != Audio && state != PlaylistState::Disabled && previousState != PlaylistState::Disabled &&
            (previousState == PlaylistState::AudioOnly || state == PlaylistState::AudioOnly)) {
        ...
        emit displayMessage(
            i18n("Cannot update clip (time: %1, track: %2)", info.startPos.frames(pCore->getCurrentFps()), m_timeline->getTrackInfo(info.track).trackName),
            ErrorMessage);
        return;
    }
}
...
void CustomTrackView::reloadTransitionLumas()
{
    ...
    for (int i = 0; i < itemList.count(); ++i) {
        if (itemList.at(i)->type() == TransitionWidget) {
            ...
            if (transitionXml.attribute(QStringLiteral("id")) == QLatin1String("luma") &&
                transitionXml.attribute(QStringLiteral("tag")) == QLatin1String("luma")) {
            ...
            if (transitionXml.attribute(QStringLiteral("id")) == QLatin1String("composite") &&
                transitionXml.attribute(QStringLiteral("tag")) == QLatin1String("composite")) {
                QDomNodeList params = transitionXml.elementsByTagName(QStringLiteral("parameter"));
                for (int j = 0; j < params.count(); ++j) {
                    QDomElement e = params.item(j).toElement();
...
void CustomTrackView::updateProjectFps()
{
    ...
    for (int i = 0; i < itemList.count(); ++i) {
        if (itemList.at(i)->parentItem() == nullptr && (itemList.at(i)->type() == AVWidget || itemList.at(i)->type() == TransitionWidget)) {
            AbstractClipItem *clip = static_cast<AbstractClipItem *>(itemList.at(i));
            clip->updateFps(pCore->getCurrentFps());
...
void CustomTrackView::checkTrackSequence(int track)
{
    ...
    for (int i = 0; i < selection.count(); ++i) {
        if (selection.at(i)->type() == AVWidget) {
            ClipItem *clip = static_cast<ClipItem *>(selection.at(i));
            int start = clip->startPos().frames(pCore->getCurrentFps());
            int end = clip->endPos().frames(pCore->getCurrentFps());
...
int CustomTrackView::insertZone(TimelineMode::EditMode sceneMode, const QString &clipId, QPoint binZone)
{
    ...
    if (KdenliveSettings::useTimelineZoneToEdit()) {
        if (clp->hasLimitedDuration()) {
            ...
            timelineLength = qMin(timelineLength, (int)clp->duration().frames(pCore->getCurrentFps()) - binZone.x());
        } else if (timelineLength > clp->duration().frames(pCore->getCurrentFps())) {
            // Update source clip max length
            clp->setProducerProperty(QStringLiteral("length"), timelineLength + 1);
        }
    }
    info.startPos = GenTime(timelineZone.x(), pCore->getCurrentFps());
    info.cropStart = GenTime(binZone.x(), pCore->getCurrentFps());
    info.endPos = info.startPos + GenTime(KdenliveSettings::useTimelineZoneToEdit() ? timelineLength : binLength, pCore->getCurrentFps());
    info.cropDuration = info.endPos - info.startPos;
    ...
    if (sceneMode == TimelineMode::OverwriteEdit) {
        ...
        extractZone(QPoint(timelineZone.x(), info.endPos.frames(pCore->getCurrentFps())), false, QList<ItemInfo>(), addCommand);
    } else {
    ...
    if (KdenliveSettings::splitaudio()) {
        if (extractVideo) {
            ...
            new AddTimelineClipCommand(this, clipId, info, EffectsList(), cType == Audio ? PlaylistState::Original : PlaylistState::AudioOnly, true, false,
                                       false, addCommand);
        } else {
...
{
    EffectsController::initTrackEffect(effect);
    auto *effectCommand = new QUndoCommand();
    QString effectName;
    if (effect.tagName() == QLatin1String("effectgroup")) {
        effectName = effect.attribute(QStringLiteral("name"));
    ...
    if (effect.tagName() == QLatin1String("effectgroup")) {
        ...
        for (int j = 0; j < effectlist.count(); ++j) {
            ...
            if (trackeffect.attribute(QStringLiteral("unique"), QStringLiteral("0")) != QLatin1String("0") &&
                m_timeline->hasTrackEffect(ix, trackeffect.attribute(QStringLiteral("tag")), trackeffect.attribute(QStringLiteral("id"))) != -1) {
        ...
        if (effect.attribute(QStringLiteral("unique"), QStringLiteral("0")) != QLatin1String("0") &&
            m_timeline->hasTrackEffect(ix, effect.attribute(QStringLiteral("tag")), effect.attribute(QStringLiteral("id"))) != -1) {
            emit displayMessage(i18n("Effect already present in track"), ErrorMessage);
            delete effectCommand;
            return;
        }
...
void CustomTrackView::adjustEffects(ClipItem *item, const ItemInfo &oldInfo, QUndoCommand *command)
{
    ...
    if (!effects.isEmpty()) {
        ...
        while (i != effects.constEnd()) {
            new EditEffectCommand(this, item->track(), item->startPos(), i.value(), item->effect(i.key()),
                                  i.value().attribute(QStringLiteral("kdenlive_ix")).toInt(), true, true, false, command);
            ++i;
        }
    }
}
...
{
    ClipItem *clip = getClipItemAtStart(GenTime(startPos, pCore->getCurrentFps()), track);
    if (clip == nullptr) {
    ...
    if (effect.attribute(QStringLiteral("id")) == extra.value(QStringLiteral("finalfilter"))) {
        ...
        EditEffectCommand *command =
            new EditEffectCommand(this, clip->track(), clip->startPos(), effect, newEffect, clip->selectedEffectIndex(), true, true, true);
        m_commandStack->push(command);
        emit clipItemSelected(clip);
    } else {
...
void CustomTrackView::slotImportClipKeyframes(GraphicsRectItem type, const ItemInfo &info, const QDomElement &xml, QMap<QString, QString> keyframes)
{
    ...
    /*QPointer<KeyframeImport> import = new KeyframeImport(srcInfo, info, keyframes, m_document->timecode(), xml, this);
...
{
    return ((m_selectionGroup != nullptr) || (m_dragItem != nullptr));
}

void CustomTrackView::exportTimelineSelection(QString path)
{
    if ((m_selectionGroup == nullptr) && (m_dragItem == nullptr)) {
    ...
    auto *newTractor = new Mlt::Tractor(*(m_document->renderer()->getProducer()->profile()));
    ...
    for (int i = 0; i < children.count(); ++i) {
        ...
        int pos = it->startPos().frames(pCore->getCurrentFps());
        if (pos < startOffest) {
            startOffest = pos;
        }
    }
...
void CustomTrackView::importPlaylist(const ItemInfo &info, const QMap<QString, QString> &idMap, const QDomDocument &doc, QUndoCommand *command)
{
    ...
    if ((import == nullptr) || !import->is_valid()) {
        delete command;
    ...
    if (bottomTrack < 1) {
        ...
        if (playlistTracks > m_timeline->visibleTracksCount()) {
            ...
            delete command;
            emit displayMessage(i18n("Selected playlist clip needs more tracks (%1) than there are tracks in the timeline (%2).", playlistTracks,
                                     m_timeline->visibleTracksCount()),
                                MltError);
            return;
        }
    ...
    for (int i = 0; i < selection.count(); ++i) {
        ...
        if (item->type() == TransitionWidget || item->type() == AVWidget) {
            ...
            emit displayMessage(
                i18n("Not enough free track space above or below the selected playlist clip: need free room on %1 tracks to expand playlist.", playlistTracks),
                MltError);
            delete command;
            return;
        }
    ...
    for (int i = 0; i < playlistTracks; i++) {
        int startPos = info.startPos.frames(pCore->getCurrentFps());
        ...
        for (int j = 0; j < trackPlaylist.count(); j++) {
            ...
            // Ready, insert clip
            ItemInfo insertInfo;
            insertInfo.startPos = GenTime(startPos, pCore->getCurrentFps());
            int in = original->get_in();
            int out = original->get_out();
            insertInfo.cropStart = GenTime(in, pCore->getCurrentFps());
            insertInfo.cropDuration = GenTime(out - in + 1, pCore->getCurrentFps());
    ...
    while ((serv != nullptr) && serv->is_valid()) {
        if (serv->type() == transition_type) {
            ...
            if (t.get_int("internal_added") > 0) {
                ...
                transitionInfo.startPos = info.startPos + GenTime(t.get_in(), pCore->getCurrentFps());
                transitionInfo.endPos = info.startPos + GenTime(t.get_out(), pCore->getCurrentFps());
                transitionInfo.track = t.get_b_track() + bottomTrack;
                int endTrack = t.get_a_track() + bottomTrack;
                if (prop.get("kdenlive_id") == nullptr && QString(prop.get("mlt_service")) == QLatin1String("composite") &&
                    Timeline::isSlide(prop.get("geometry"))) {
...
{
    if ((m_dragItem == nullptr) || m_dragItem != trans) {
        clearSelection(false);
        m_dragItem = trans;
        m_dragItem->setMainSelectedClip(true);
...
{
    if ((m_dragItem == nullptr) || m_dragItem != clip) {
        clearSelection(false);
        m_dragItem = clip;
        m_dragItem->setMainSelectedClip(true);
        clip->setSelected(true);
...
void CustomTrackView::breakLockedGroups(const QList<ItemInfo> &clipsToMove, const QList<ItemInfo> &transitionsToMove, QUndoCommand *masterCommand, bool doIt)
{
    ...
    for (int i = 0; i < clipsToMove.count(); ++i) {
        ...
        if (clip == nullptr || (clip->parentItem() == nullptr)) {
    ...
    for (int i = 0; i < transitionsToMove.count(); ++i) {
        ...
        if (trans == nullptr || (trans->parentItem() == nullptr)) {
            continue;
        }
        // If group has a locked item, ungroup first
...
void CustomTrackView::doRipple(bool accept)
{
    if (accept) {
        ...
        // Ripple clip start
        ClipItem *second = getClipItemAtEnd(info.startPos, info.track); // - GenTime(1, pCore->getCurrentFps()), info.track);
        if (!second) {
            // Something is wrong
        ...
        if (resizePos > info.startPos.frames(pCore->getCurrentFps())) {
...
void CustomTrackView::switchTrimMode(TrimMode mode)
{
    ...
    if (trimItem) {
        info = trimItem->info();
        GenTime cursor(m_cursorPos, pCore->getCurrentFps());
        if (cursor == info.startPos) {
            ...
            int diffStart = qAbs(m_cursorPos - info.startPos.frames(pCore->getCurrentFps()));
            int diffEnd = qAbs(m_cursorPos - info.endPos.frames(pCore->getCurrentFps()));
...
{
    GenTime timeOffset(diff, pCore->getCurrentFps());
    ...
    if (canBePasted(items, timeOffset, 0, excluded)) {
        m_selectionGroup->setTransform(QTransform::fromTranslate(diff, 0), true);
        clip->setCropStart(clip->cropStart() - GenTime(diff, pCore->getCurrentFps()));
        clip->resizeEnd(clip->endPos().frames(pCore->getCurrentFps()) + diff, false);
        qCDebug(KDENLIVE_LOG) << "* * *RESIZING END: " << clip->endPos().frames(pCore->getCurrentFps()) + diff;
    } else {
...
void CustomTrackView::finishRipple(ClipItem *clip, const ItemInfo &startInfo, int diff, bool fromStart)
{
    ...
    if (fromStart) {
        // newInfo.cropStart += GenTime(diff, pCore->getCurrentFps());
    }
    ...
    m_commandStack->push(moveCommand);
    if (m_timeline->track(startInfo.track)
            ->resize_in_out(startInfo.startPos.frames(pCore->getCurrentFps()), clip->cropStart().frames(pCore->getCurrentFps()),
                            clip->cropStart().frames(pCore->getCurrentFps()) + clip->cropDuration().frames(pCore->getCurrentFps())))
