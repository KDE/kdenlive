...
QString ProjectManager::projectSceneList(const QString &outputFolder, const QString &overlayData, bool rendering)
{
    ...
    if (rendering || m_activeTimelineModel->uuid() == m_project->uuid()) {
...
void ProjectManager::openTimeline(const QString &id, const QUuid &uuid)
{
    ...
    /*if (m_project->timelineCount() == 1) {
        ...
        mainProperties.insert(QStringLiteral("kdenlive:clipname"), i18n("Sequence 1"));
        ...
        pCore->bin()->registerSequence(m_activeTimelineModel->uuid(), mainId);
    ...
    if (internalLoad) {
        ...
        if (xmlProd->type() == mlt_service_tractor_type) {
            ...
            tractor->set("id", timelineModel->uuid().toString().toUtf8().constData());
        }

        if (!constructTimelineFromMelt(timelineModel, *tractor.get(), m_progressDialog, m_project->modifiedDecimalPoint())) {
            // TODO: act on project load failure
            qDebug() << "// Project failed to load!!";
            delete timeline;
            m_autoSaveTimer.start();
        ...
        qDebug() << "::: SEQUENCE LOADED WITH TRACKS: " << timeline->tractor()->count() << "\nZZZZZZZZZZZZ";
        std::shared_ptr<Mlt::Producer> prod = std::make_shared<Mlt::Producer>(timeline->tractor());
