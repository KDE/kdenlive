...
void TimelineController::editGuide(int frame)
{
    ...
    guideModel->editMarkerGui(pos, qApp->activeWindow(), false, nullptr, false);
}
...
void TimelineController::switchGuide(int frame, bool deleteOnly, bool showGui)
{
    ...
    qDebug()<<"=== TRYING TO REACH GUIDE MODEL WITH UUID: "<<m_model->uuid();
    CommentedTime marker = pCore->currentDoc()->getGuideModel(m_model->uuid())->getMarker(GenTime(frame, pCore->getCurrentFps()), &markerFound);
    if (!markerFound) {
        if (deleteOnly) {
...
void TimelineController::initializePreview()
{
    if (m_timelinePreview) {
        ...
        m_timelinePreview = new PreviewManager(pCore->getDocument(m_model->uuid())->documentProfile(), this, m_model->m_tractor.get());
...
{
    return pCore->getDocument(m_model->uuid())->getDocumentProperty(QStringLiteral("enableTimelineZone")).toInt() == 1;
}
