...
Row {
    ...
    Repeater {
        ...
        TimelineWaveform {
            ...
            waveInPoint: clipRoot.speed < 0 ? (Math.round((clipRoot.maxDuration - 1 - clipRoot.switchedInPoint) * Math.abs(clipRoot.speed)  - (index * waveform.maxWidth / clipRoot.timeScale) * Math.abs(clipRoot.speed)) * channels) : (Math.round((clipRoot.switchedInPoint + (index * waveform.maxWidth / clipRoot.timeScale)) * clipRoot.speed) * channels)
